...
if demo_mode_is_full; then
  wait_http_or_fail "http://localhost:${SANDBOX_HOST_PORT:-29080}/readyz" 240 sandbox_startup_failed infra_preflight_sandbox
  record_service sandbox_manager ready "http://localhost:${SANDBOX_HOST_PORT:-29080}/readyz"
  require_command "kubectl get csidriver csi.juicefs.com >/dev/null" sandbox_startup_failed infra_preflight_csi "csi.juicefs.com missing"
  require_command "kubectl get deploy sandbox-manager -n agentsmith-sandbox >/dev/null" sandbox_startup_failed infra_preflight_sandbox "sandbox-manager deploy missing"
  require_command "kubectl get cronjob sandbox-manager-cleaner -n agentsmith-sandbox >/dev/null" sandbox_startup_failed infra_preflight_sandbox "sandbox-manager-cleaner missing"
...
if ! curl -fsS "${HOST_LOCAL_API_BASE_URL}/api/v1/me/profile" \
