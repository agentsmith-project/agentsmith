...
AGENT_TASK_RUNNER_BASE_IMAGE="${AGENT_TASK_RUNNER_BASE_IMAGE:-$(runner_release_base_image agent-task "${RELEASE_ID}")}"
VERIFY_RUNNER_BASE_IMAGE="agentsmith-verify-runner-base:${RELEASE_ID}"
APP_IMAGE="${IMAGE_PREFIX}/agentsmith-app:${RELEASE_ID}"
AGENT_TASK_RUNNER_IMAGE="${AGENT_TASK_RUNNER_IMAGE:-$(runner_release_image agent-task "${RELEASE_ID}" "${IMAGE_PREFIX}")}"
...
build_runner_image agent-task "${AGENT_TASK_RUNNER_BASE_IMAGE}" "${AGENT_TASK_RUNNER_IMAGE}" "${DOCKER_BUILD_PROXY:-}" "1" "1" "${APP_SOURCE_DIR}"
docker_build_local \
