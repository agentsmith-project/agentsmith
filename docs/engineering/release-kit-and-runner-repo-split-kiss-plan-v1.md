...
10. P6-lite 文档/旧引用清理默认按小切片 + focused diagnostics 推进，使用 doc/static guard + targeted contracts；新增规则时优先删除或归档旧重复规则。只有改 release/runtime/product readiness 路径才升级到 `npm run product:ready` 或发布级重门禁。
...
2. AgentSmith link-level handoff validator 与 P6-lite summary/status 降噪已完成；`product:ready` / `product:status` 默认不展示 transition-only unified deploy diagnostics。`release:ready` / `release:status` 只保留为 deprecated transition aliases / 过渡 alias。
...
5. 当前边界已取代早期过渡语义：`npm run product:ready` 是 AgentSmith product readiness / local complete / current product gate，内部覆盖 full visual、backend-real release 和 terminal aggregate；`npm run release:ready` 只保留为 deprecated transition alias / 过渡 alias。local-kind / unified deploy / product-flow deploy commands 只保留为 transition-only focused diagnostics / 过渡期专项诊断，不属于 AgentSmith 产品门禁结论。online/airgap deployment、package、发布测试和 operator runbook 的 verdict 归 release-kit repo-local gate/evidence，AgentSmith 只保留 product readiness、product image / managed runner image handoff truth、local full test、release contract 和 thin adapter。
...
- `npm run verify`、`npm run product:ready`、product flows、visual、backend-real 等产品验收真相。
...
当前边界保持不变：focused diagnostics 不等于 readiness；AgentSmith `product:ready` 不给 deployment/package/operator verdict；release-kit repo-local gate/evidence 才能给部署、发布包和 operator 结论。

当前默认下一步只保留 P6-lite 文档/旧引用归档清理。
已完成的 release-kit focused/candidate surface（含四象限 adoption inputs、scoped acceptance / unsigned evidence）、AgentSmith handoff/status 降噪、runner locked projection semantic smoke 和 runner locked safety smoke 都见 evidence log；它们不再作为待办，也不能升级为 formal release engineering verdict、operator verdict、package verdict、release readiness、backend-real/full semantics，或接回 AgentSmith `product:ready`。
...
2. local-kind evidence 不能代替 `existing_kubernetes` evidence；反过来也不能把 operator smoke 自动算成默认 `product:ready` 结论。
...
adoption 聚合与 AgentSmith handoff 输入事实，不是 `product:ready`、
...
| Runner release manifest | `agentsmith-runner` | runner repo CI artifact；P5.3a skeleton/checker/start-guard、image smoke、publish manifest 和 AgentSmith positive fixture adoption 已完成，细节见 evidence log | runner repo release workflow；manifest 绑定 run/job/artifact URI、image digest 和 AgentSmith contract artifact package URI | AgentSmith runner lock checker；manifest skeleton/fail-closed start guard；AgentSmith manifest/lock adoption 与 release contract runner digest adoption | `image.id` 不是 `agentsmith-runner`、保留 `agent-task-runner` 旧别名、缺 image digest 或 `artifact_provenance`、`contract_artifact` 未绑定 P5.2 字段 `package_uri` / `package_sha256` / `package_integrity` / `descriptor_subject_sha256`、发明 `descriptor_uri` / `descriptor_sha256`、contract version 不匹配、producer repo 不是 `agentsmith-runner`、把 manifest/lock/contract adoption 写成 runtime/backend-real/release-kit readiness |
| Runner image lock | AgentSmith | canonical release artifact `release/agentsmith-runner-image.lock`；正式 lock identity 为 `agentsmith-runner`；旧 `agent-task-runner-image.lock` 不作为 positive fixture，`image_id=agent-task-runner` 只允许出现在负向测试/短期待删说明 | AgentSmith manifest/lock adoption PR | AgentSmith runner image lock checker、release contract generator、release boundary checker；backend-real 是后续专项 | lock 与 runner manifest 不一致、release contract `managed_runner_image` / `managed_runner` inventory 与 lock 不一致、接受 caller 自报 `managed_runner_image`、接受 `agent-task-runner` 作为正式 lock identity、保留旧别名 |
...
6. 保持 `npm run product:ready` 作为 AgentSmith product readiness / local complete / current product gate 入口不变，避免同时改产品验收和部署工具归属。
...
- `product:ready` 仍使用当前产品证据闭环，不消费 deploy/package/operator verdict。

### P2. Release Kit Online MVP

目标：独立 repo 先跑通真实 Kubernetes online deploy，不碰产品验收；kind 只作为可选 rehearsal。

当前状态：P2 online target-registry confirmed apply/evidence spine、operator
signoff intake focused guard、operator-preloaded registry prerequisite binding 和
最小 online adoption 聚合已完成；细节、commit、CI run 与本地命令见
[Evidence log reference](archive/release-kit-and-runner-repo-split-evidence-log-v1.md)。
这些 focused 证据只覆盖 online gate positive path、canonical
`image-map,registry-presence` producer sequence、operator intake 机器绑定和
`readiness=false` adoption summary；不等于 registry mirror/login/push/pull、
deploy adoption、cloud provisioning、full release-kit verdict、formal release
gate、operator signature/identity/full verdict、AgentSmith product-flow evidence
...
12. online release-kit verdict 必须覆盖 `use_existing` 和
    ...
    link-level handoff validator 已完成，但未接入 `product:ready` 或
...
- P2 过渡说明以当前边界为准：AgentSmith `product:ready` 是 product readiness / local complete / current product gate，不要求 dependencies/images/rollout/product-flow deploy evidence；这些 unified deploy outputs 只保留为过渡期专项诊断，直到 P2/P3/P6 收口时从 AgentSmith active status/workflow 删除或隐藏。这里不暗示未来 AgentSmith release campaign 会继续消费它们。
...
dynamic release contract image closure、operator-owned archive probe /
image-loader digest alignment、bundle packaging、substrate pack validation 和
consume/deployment chain；不证明 registry execution、offline install/apply/smoke、
deploy/package/operator verdict、release engineering gate、airgap ready 或 release
readiness。
...
当前状态：P4 已完成，细节见 evidence log。正式 artifact 是外部
...
当前状态：P5.0-P5.3b、focused image smoke、publish manifest、AgentSmith
manifest/lock adoption、release contract runner digest adoption、release-kit
managed runner image closure consumption、request-scoped projected dependencies
contract/env wiring、focused task-execution smoke 和 locked runner projection
semantic smoke 已完成；细节、commit、CI run、digest 和本地命令见
[Evidence log reference](archive/release-kit-and-runner-repo-split-evidence-log-v1.md)。
...
1. Runner repo 消费已下载的 `@mbos/agent-runner-contract` artifact root；未来若需要真实 URI downloader，归 future downloader/adoption/provenance work，不回写成 P5.1/P5.2 已完成事实。
...
以及禁止 runner repo 读取 AgentSmith source / release kit 从 source build runner。它不证明 runner manifest、lock 和 release contract digest match；digest adoption proof 已由 `contracts:check-runner-image-lock -- --adoption --manifest ... --handoff-report ...` 等 adoption gate 证据收口。local-kind build、本地开发启动入口、backend-real/skills diagnostics 都只能是过渡诊断，不能作为 release proof。
...
- P5.0-P5.3b、focused image smoke、publish manifest、AgentSmith manifest/lock adoption、release contract runner digest adoption、release-kit managed runner image closure consumption、request-scoped projected dependencies wiring 与 locked runner projection semantic smoke 的完成证据见 evidence log；active plan 只保留它们不是 release readiness/backend-real/full runtime semantics 的边界。source-boundary guard 仍只允许正式路径 import `@mbos/agent-runner-contract`。
...
- 只有改 release/runtime/product readiness 路径才升级到
  `npm run verify -- --goal=pr --run`、`npm run product:ready` 或发布级重门禁。
...
12. 当前 `existing-cluster` 仍按 Docker substrate/IP-only diagnostic 降级命名；
    ...
    任何把 `existing-cluster` 并入 AgentSmith `product:ready` 或真实 cloud/airgap evidence 的路径都失败。

### 后续门禁 / 阶段收口 invariant

必须确认：

1. 已完成的 P2/P3/P5 focused/candidate/adoption 切片只解锁后续专项，历史 commit、CI run、digest 和命令见 evidence log；它们不等于 airgap ready、release readiness、deployment/package/operator verdict 或 AgentSmith product gate。只有新增 repo、新增 release gate family 或新增职责边界时，才先做 bootstrap-only/docs-governance-first PR；已 bootstrap repo 的普通功能切片走最小 contract/test/evidence，不先做仪式性治理 PR。quick gate is not release readiness；formal release readiness comes from the repo-local release gate。
...
4. P2 formal release gate 前，repo-local required CI 必须覆盖 source-boundary、remote identity、provenance、digest-only、host/TLS/pull-secret/storage/substrate secret-ref 输入、online gate steps、target/adopted ref strict live check、no-op fail-fast、operator runbook，以及 operator signoff intake 与 confirmed apply online gate report 的机器绑定。target-registry apply 必须带 `--registry-probe`，producer sequence 必须是 canonical `image-map,registry-presence`；source-registry apply 不受影响，target-registry server-dry-run 不要求且不允许 probe。registry mirror/login/push/pull 仍必须显式补齐。operator signature/identity/full verdict 只有出现明确客户/合规/发布消费方需要时再做，真实 deploy/smoke 站稳前不推进；AgentSmith product-flow evidence 仍归 AgentSmith，不进入 release-kit signoff。
...
3. AgentSmith 只给 product readiness / local complete / current product gate；`product:ready` 不给 deployment/package/operator verdict，也不把 release-kit focused output 当产品门禁。
...
5. 高信号边界继续由 static guard 兜底：`kind` / `local-kind` 不能写成正式 release target/prerequisite；release-kit focused evidence 不能写成 AgentSmith `product:ready`；deployment/operator verdict 不能接回 AgentSmith `product:ready`。
