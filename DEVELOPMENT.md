...
- AgentSmith Files collection projection 与 operation terminal truth 分离：collection 只表达当前 list-visible 集合；active save point operation 可把空集合或 list pending 投影为 typed pending；terminal failed/succeeded 通过 operation resource 与后端 list-visible truth 暴露，不用 collection 伪造 last-operation failed 或长期 pending。accepted operation 必须持续可查询到 active/terminal 状态；被 accepted 后只能收敛为 list-visible terminal success、typed pending continuation 或 typed failed，不得从 projection 或 operation resource 静默消失。
...
- 机器可读报告语境：post-deploy product smoke handoff `npm run test:unified-deploy:product-flows` is only the focused aggregate diagnostic. The AgentSmith-owned canonical report producer is the product-flow lane (`lane:unified-deploy:product-flows`), with `UNIFIED_DEPLOY_RELEASE_CONTRACT` or `AGENTSMITH_RELEASE_CONTRACT_PATH` pointing to the downloaded `agentsmith-release-contract.json`, `UNIFIED_DEPLOY_RELEASE_SITE_ENV` pointing to the deployed target site env, `UNIFIED_DEPLOY_RELEASE_SUBSTRATE_TRUTH` pointing to the same deployed target neutral `agentsmith.substrate-connection.truth/v1` JSON, runtime-only substrate env projection supplied separately for product-flow execution, and `UNIFIED_DEPLOY_RELEASE_ROOT_DIR=<ga-smoke-evidence-root>`. It writes `<ga-smoke-evidence-root>/post-deploy-product-smoke/post-deploy-product-smoke-report.json` for the release-kit final GA facade (`bash scripts/operator-release.sh --ga-report`); it is not part of default `product:ready` / release-full.
...
7. 机器可读报告语境：Post-deploy product smoke handoff uses the product-flow lane (`lane:unified-deploy:product-flows`) as an AgentSmith-owned producer for repeated `--post-deploy-product-smoke-report` inputs to `bash scripts/operator-release.sh --ga-report`, not as a `product:ready` / release-full step. Configure the downloaded release contract with `UNIFIED_DEPLOY_RELEASE_CONTRACT` or `AGENTSMITH_RELEASE_CONTRACT_PATH`, the deployed target site env with `UNIFIED_DEPLOY_RELEASE_SITE_ENV`, the same deployed target neutral `agentsmith.substrate-connection.truth/v1` JSON with `UNIFIED_DEPLOY_RELEASE_SUBSTRATE_TRUTH`, runtime-only substrate env projection separately for product-flow execution, and the output root with `UNIFIED_DEPLOY_RELEASE_ROOT_DIR`.
