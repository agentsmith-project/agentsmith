...
if demo_mode_is_full; then
  ...
  gate_record_preflight_check "${VERIFY_EVIDENCE_DIR}" "managed_runner_control_plane" "passed" "sandbox manager and csi ready"
fi
...
record_task_summary "{\"workspace_id\":\"ws_default\",\"project_id\":\"${PRESET_PROJECT_ID}\",\"line_kind\":\"demo_deploy_verify\",\"runner_mode\":\"managed\"}"
...
state_set verify.runner_mode managed
...
docker run --rm \
  ...
  -e INTEGRATION_AGENT_TASK_RUNNER_MOUNT_READY_TIMEOUT_MS="${MBOS_AGENT_JUICEFS_MOUNT_READY_TIMEOUT_MS:-120000}" \
