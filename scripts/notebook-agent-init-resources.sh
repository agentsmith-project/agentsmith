...
agent_resp="$(api_curl -X POST "${PROJECT_BASE}/agents" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H 'Content-Type: application/json' \
  -d "$(node -e 'console.log(JSON.stringify({name:process.argv[1], mode:"external", interaction_kind:"notebook", config:{runner_runtime:"dev_direct"}, execution_preferences:{notebook:{endpoint_id:process.argv[2], wire_api:process.argv[3], model:process.argv[4]}}, capabilities:{streaming_completion:true,multimodal_completion:false}}))' \
...
echo "[init] agent key written to $(backend_real_state_file)"
