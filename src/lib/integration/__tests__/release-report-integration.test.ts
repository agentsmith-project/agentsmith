/**
 * M3 Integration Tests: Release Report Generation
 *
 * Epic D1 (Release Report Automation) Integration Tests
 * Testing: Release report generation, failure categorization
 *
 * M3 Goal: Verify release reports work with all epics.
 */

import { describe, it, expect, beforeEach } from 'vitest';
import type { ReleaseReport } from '../../../../scripts/release/types';
import type { FailureCategory as _FailureCategory } from '../../../../scripts/release/types';

describe('M3 Integration: Release Report Generation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  // Integration Test 1: Release report includes all epic results
  it('should aggregate smoke test results from all epics', async () => {
    const mockReleaseReport: Partial<ReleaseReport> = {
      metadata: {
    ...
    expect(mockReleaseReport.metadata).toBeDefined();
    expect(mockReleaseReport.execution).toBeDefined();
    expect(mockReleaseReport.summary).toBeDefined();
  });
  ...
  // Integration Test 3: Release report generates both JSON and Markdown
  it('should output release report in JSON and Markdown formats', async () => {
    const mockReportData = {
      title: 'Release Report - v1.0.0',
      status: 'passed',
      timestamp: new Date().toISOString(),
    };
    ...
    expect(mdHeader).toContain('# Release Report');
  });
  ...
  // Integration Test 5: Release report includes commit metadata
  it('should include commit range and environment metadata', async () => {
    const mockReport: Partial<ReleaseReport> = {
      metadata: {
  ...
  // Integration Test 7: Full release report generation workflow
  it('should generate complete release report with all sections', async () => {
    const mockFullReport: Partial<ReleaseReport> = {
      metadata: {
        timestamp: new Date().toISOString(),
        duration_ms: 5000,
        environment: {
