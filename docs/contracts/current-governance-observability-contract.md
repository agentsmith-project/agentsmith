...
- `status_projection_schema`: read-only status projection. It may point to aggregate status and stage diagnostics, but it must not produce `release_verdict` or `automated_release_verdict`; terminal summaries rendered through the projection must stay inside the redaction boundary.
