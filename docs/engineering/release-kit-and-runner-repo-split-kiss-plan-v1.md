...
Status: `p1_1_artifact_producer_passed_p2_online_target_registry_apply_evidence_spine_done_p2_registry_presence_focused_done_p2_operator_preloaded_registry_prereq_binding_done_operator_signoff_intake_guard_done_p3_airgap_bundle_render_check_done_p3_app_current_inventory_closure_done_p3_airgap_image_archive_materiality_focused_done_p5_1_start_guard_done_p5_2_formal_artifact_handoff_done_p5_3a_runner_release_manifest_skeleton_done_agentsmith_p5_3a_machine_contract_alignment_done`
Date: 2026-05-26
...
image build/publish、真实 adoption 或 release readiness。P2 online target-registry
...
readiness。此前 AgentSmith commit `7cf783c2` 已推送；远端 Contracts Check run `26447963233`
...
handoff，也不是 P5.3a 后直接发布，而是 P2 full
...
load/import/offline install/deploy smoke 收口，以及 P5 runtime migration /
...
仍不迁 runner runtime，也不宣称 registry mirror/login/push/pull、
...
install/deploy smoke、image build/publish、AgentSmith adoption、lock update、
...
14. DeepSeek/LLM real lane 没有 tracked changes；AgentSmith defaults 和 ignored local env 使用 DeepSeek endpoint/model，LLMUP real compatibility smoke 15 passed / 0 failed / 1 skipped，未提交 secret。
15. AgentSmith commit `7cf783c2` 已推送；此前远端 Contracts Check run `26447963233` 因 GitHub Actions checkout/auth 403 失败，不是治理脚本失败。
...
4. Runner 复审结论：不要先搬 runtime。P4 AgentSmith formal artifact producer/checker 已完成，正式 artifact 是外部 `runner-contract-artifact.json` + tgz；P5.0 runner repo consumer diagnostic skeleton 已完成并可消费正式 artifact；P5.1 start guard/CI 化已完成；P5.2 formal artifact handoff 已完成并验证，只证明 AgentSmith producer 产物能被 runner repo consumer 消费；P5.3a release manifest skeleton/checker/start-guard 集成已完成，只证明 manifest skeleton 可校验、可接入 start guard、full release gate fail-closed。下一步仍不是直接发布，而是 P5 runtime migration / image build CI / adoption lock 后续专项，按 KISS 小切片推进。
...
| Runner contract | AgentSmith shared-contract flow | `@mbos/agent-runner-contract` package (`packages/agent-runner-contract/src`) schema/types/fixtures，以及 P4 产出的外部 `runner-contract-artifact.json` + tgz；包内 manifest 是 package manifest v1；`local_pack_manifest` 只作为负向测试输入；已移除旧包 `@mbos/agent-runner` 是 pre-GA 旧输入，正式路径默认拒绝，只能出现在第 3.2 节定义的负向测试、过渡期专项诊断或短期迁移说明里，P5 runtime migration / image build CI / adoption lock 后删除或归位 | AgentSmith runner contract artifact producer/checker from `@mbos/agent-runner-contract` | AgentSmith API、runner repo、AsyncAPI/doc checks、artifact-root install/import consumer test | 缺 artifact URI/digest/integrity/provenance、AsyncAPI 漂移、已移除旧字段、unsupported protocol version、手工复制类型、正式路径接受 `local_pack_manifest` |
...
build/publish、AgentSmith adoption、lock update 或 release readiness。下一步仍
不是直接发布，而是 P5 runtime migration / image build CI / adoption lock 后续
专项，按 KISS 小切片推进。
...
6. P5.1/P5.2/P5.3a 通过只解锁 runtime 迁移、image build CI 和 adoption lock 后续专项，不解锁 release/adoption；env/credential/runtime smoke、Dockerfile、image build/publish、AgentSmith lock update 和 adoption 仍必须在后续 P5 runtime/conformance/adoption gates 里验证。
7. 下一阶段再迁入 `packages/agent-task-runner`、builtin skills runtime、runner Dockerfile 和 runner 单测；`packages/agent-task-runner` 是 runtime migration 前的当前代码事实/迁移对象，不是 release identity。
...
9. P5.1 repo-local start guard 已有 consumer / start-guard / full-gate-fail-closed 本地证据；P5.2 AgentSmith workflow handoff 已有同 run artifact 下载和 runner repo `--contract-consumer` 证据；P5.3a 已有 manifest skeleton/checker/start-guard 证据；runtime 迁移后再扩展 HOME/TASK_HOME、request-scoped env projection、credential non-persistence、builtin skill tests、Docker build 和启动缺 env fail-fast smoke。P5 后 source-boundary guard 只允许正式路径 import `@mbos/agent-runner-contract`，其他 `@mbos/*` import 失败。
...
12. Runner adoption 顺序固定为：P5.0 consumer diagnostic skeleton -> P5.1 start guard/CI 化和负向 fixtures -> P5.2 formal artifact handoff -> P5.3a release manifest skeleton/checker/start-guard -> runtime migration / image build CI 通过 -> runner repo release manifest/image digest -> AgentSmith 更新 lock -> AgentSmith release contract 输出锁定 digest -> release kit 消费 release contract。
13. P5 runtime 迁移前补 monorepo runner adapter inventory，并逐项迁移：local-kind runner image build、API 默认 managed runner image、internal agent pod health/imageID probe、`agent:task-runner` dev script、skills diagnostics、`buildAgentRuntimeEnv` ownership、P5 manifest/lock adoption 后的 release contract runner digest。`agent:task-runner` dev script 是当前代码事实/迁移对象，不是 release identity。
...
- P5.0 consumer diagnostic skeleton 已通过；P5.1 start guard/CI 化已通过；P5.2 formal artifact handoff 已通过；P5.3a release manifest skeleton/checker/start-guard 已通过。后续 P5 runtime migration / image build CI / adoption lock 阶段，source-boundary guard 只允许正式路径 import `@mbos/agent-runner-contract`。
...
### P2 online spine / registry prerequisite binding / operator signoff intake / P3 focused diagnostics/inventory closure / P5.3a manifest skeleton 完成后的后续门禁
...
1. P2 online target-registry confirmed apply/evidence spine、
   ...
   `--airgap-bundle-render-check` focused diagnostic、P3 app-current image
   inventory closure、P3 `--airgap-image-archive-check` materiality focused
   diagnostic、P5.1 start guard、P5.2 formal artifact handoff 和 P5.3a
   runner release manifest skeleton/checker/start-guard 集成已有完成证据，但只解锁后续专项；
...
6. P4/P5.0 已证明 runner contract artifact 有 URI/digest/integrity/provenance
   ...
   binding，不是远端 artifact 下载证明。runner release manifest adoption 仍必须比对 image digest、
   contract artifact digest/provenance 和 lock。
...
8. P5.3a 通过后，下一步仍不是直接发布；runner runtime 迁移、image build CI、
   AgentSmith adoption lock 和 release-kit image inventory 串联必须按后续 KISS
   小切片推进。runtime 迁出并通过 image build/adoption gate 后，才进入正式
...
manifest skeleton/checker/start-guard 已完成 -> P5 runtime migration / image
build CI / adoption lock 后续专项 -> P6 收口。P4 runner contract artifact
...
release manifest skeleton/checker/start-guard 已完成。P2 online spine、P3 render-check、P3 app-current inventory
...
lock update 或 release readiness；P5 runtime migration / image build CI /
adoption lock 完成前也不能删除 monorepo runner build 或宣称最终发布包
闭环完成。
...
mechanism 的 image load/import/offline install/deploy smoke 和 P5 runtime migration /
...
signature/identity/full verdict（正式签名验证/身份/完整 verdict）、AgentSmith product-flow evidence 收口、airgap ready 或 release readiness。DeepSeek/LLM real lane 没有 tracked changes；AgentSmith defaults 和 ignored local env 使用
...
install/deploy smoke 收口，以及 P5 runtime migration / image build CI /
adoption lock 后续专项；下一步仍不是直接发布。旧
