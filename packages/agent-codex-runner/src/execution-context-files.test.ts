...
import { applyExecutionContextFiles } from './execution-context-files.js';

describe('execution-context-files', () => {
  it('writes files to relative paths under cwd', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'runner-execution-files-'));
    const result = await applyExecutionContextFiles(cwd, [
      {
  ...
  it('rejects path traversal', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'runner-execution-files-'));
    await expect(
      applyExecutionContextFiles(cwd, [
        {
          relative_path: '../outside.txt',
          content: 'x',
        },
      ]),
    ).rejects.toThrow(/execution_file_path_outside_workspace|execution_file_path_escape_detected/);
  });
});
