...
const usageColumns = [
  ...
  columnHelper.accessor('cost', {
    ...
    cell: (info) => (
      <div className="flex items-center gap-2">
        <DollarSign className="w-4 h-4 text-icon-default flex-shrink-0" />
        <span className="text-primary text-sm font-medium">
  ...
  columnHelper.accessor('breakdown', {
    ...
    cell: (info) => {
      ...
      if (!breakdown) return <span className="text-tertiary text-sm">-</span>;
      return (
        <div className="flex flex-col gap-1 text-xs">
          {breakdown.agent_calls && (
            <span className="text-tertiary">Agent: {breakdown.agent_calls}</span>
          )}
          {breakdown.tool_calls && (
            <span className="text-tertiary">Tool: {breakdown.tool_calls}</span>
          )}
          {breakdown.api_calls && (
            <span className="text-tertiary">API: {breakdown.api_calls}</span>
          )}
        </div>
      );
    },
...
export default function UsagePage({ params }: UsagePageProps) {
  ...
  const stats = [
    { label: 'Total Requests', value: (kpi.total_requests || 0).toLocaleString(), icon: Activity, color: 'text-accent' },
    { label: 'Total Errors', value: (kpi.total_errors || 0).toLocaleString(), icon: TrendingUp, color: 'text-error' },
    { label: 'Total Tokens', value: (kpi.total_tokens || 0).toLocaleString(), icon: BarChart3, color: 'text-success' },
  ];
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-tertiary mt-1">Monitor API usage and performance metrics</p>
      </div>
      ...
      {kpiLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {stats.map((stat) => (
              <div key={stat.label} className="p-6 rounded-md border border-border bg-surface">
          ...
          <div className="p-6 rounded-md border border-border bg-surface mb-8">
            ...
            <div className="space-y-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-primary">{kpi.active_agents || 0}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-tertiary">Online Agents</span>
                <span className="text-primary">{kpi.online_agents || 0}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-tertiary">Queued Turns</span>
                <span className="text-primary">{kpi.queued_turns || 0}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-tertiary">Running Turns</span>
                <span className="text-primary">{kpi.running_turns || 0}</span>
              </div>
            </div>
          </div>
