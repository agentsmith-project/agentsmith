...
Status: `p1_1_artifact_producer_passed_p2_online_target_registry_apply_evidence_spine_done_p2_registry_presence_focused_done_p2_operator_preloaded_registry_prereq_binding_done_operator_signoff_intake_guard_done_p3_airgap_bundle_render_check_done_p3_app_current_inventory_closure_done_p3_airgap_image_archive_materiality_focused_done_p3_airgap_image_load_import_focused_done_p3_substrate_pack_focused_done_p5_1_start_guard_done_p5_2_formal_artifact_handoff_done_p5_3a_runner_release_manifest_skeleton_done_agentsmith_p5_3a_machine_contract_alignment_done_p5_3b_runner_runtime_fast_first_half_done_p5_3b_projection_only_boundary_fix_done_p5_3b_runner_boundary_closure_done_p5_runner_image_smoke_done_p5_runner_publish_manifest_evidence_done_agentsmith_manifest_lock_adoption_done_release_contract_runner_digest_adoption_done_release_kit_managed_runner_image_closure_consumption_done_agentsmith_support_api_projection_consistency_done`
Date: 2026-05-27
...
negative/fail-fast，不是 legacy 成功路径。本地 evidence：
...
runner runtime/backend-real/task execution/Codex smoke、release readiness 或
airgap ready。
...
5. Pre-GA 旧输入/旧路径/旧命名不背长期正式路径：项目整体仍 pre-GA，旧命名、旧路径、旧入口、旧文档/旧脚本引用、旧 env/profile 别名、已移除旧包或已移除字段默认不保留。正式路径不能接就删除或 fail fast；确实需要短期迁移桥时，只能作为 pre-GA 旧输入、临时迁移线索、过渡期专项诊断、operator 短期说明或后续删除清单存在，并按第 3.2 节写清删除条件、删除时机/阶段、owner 和验收证据，且不能成为长期发布/部署契约。
...
`--substrate-pack-check` focused diagnostic、release-kit operator signoff intake focused guard、P5.1 runner
...
projection contract consistency 当前切片已完成。release-kit 已消费
...
smoke、substrate installer、cloud provisioning、DB/bucket/realm 创建、runner
runtime/backend-real/task execution/Codex smoke、airgap ready 或 release readiness。
...
25. AgentSmith support API / projection contract consistency 当前切片已完成：OpenAPI 已补齐 `/api/v1/context` GET、`/api/v1/context/list` GET、`/api/v1/context/managed-credentials/{provider}/refresh` POST 的 200 response schema；`check-runner-support-api-projections` 对三处 response schema 与 `@mbos/agent-runner-contract` 的 `CONTEXT_ENTRY_PROJECTION_JSON_SCHEMA` 做严格比较，`description` 不再忽略；gate 递归拒绝 `context_store`、`writable_scopes`、`managed_credential_refresh`、`credential_files`、`user_bearer_token`；focused 检查 `/api/v1/context` 和 `/api/v1/context/list` 的 `scope` query enum，retired `user` scope 只作为 negative/fail-fast，不是 legacy 成功路径。本地 evidence：`npm run test:run -- scripts/contracts/check-runner-support-api-projections.test.ts packages/agent-runner-contract/src/support-api-projections.test.ts packages/api-entry-node/src/context-route-handler.test.ts` passed（42 tests passed）；`npm run contracts:check-runner-support-api-projections`、`npm run contracts:check-openapi`、`npm run openapi:check-generated`、`npm run contracts:check` 和 `git diff --check` passed。该切片不是 runner runtime/backend-real/task execution/Codex smoke、release readiness 或 airgap ready。
...
fail fast。旧命名、旧路径、旧入口、旧文档/旧脚本引用、旧输入、旧 shim 入口
...
一致性也已由当前 gate 切片收口。下一步不是从 manifest/lock adoption 或该切片
直接跳到 release readiness，而是 P2/P3 deployment/operator/adoption 和
runtime/backend-real/task execution/Codex smoke 后续专项，按 KISS 小切片推进。
...
release readiness，且不是 AgentSmith 侧 support API / projection contract 一致性
收口的证据；该一致性已由后续 AgentSmith gate 切片完成；
...
success）；AgentSmith pre-GA 旧输入/旧路径/旧命名不保留长期正式路径文档澄清已完成（commit
`66bf231e docs: clarify pre-GA old-input fail-fast rule`，remote Contracts Check run
...
收口：`image.id` 使用 `agentsmith-runner`，不保留 `agent-task-runner` 旧别名；
...
release contract digest adoption、release readiness，也不证明当时的 AgentSmith 侧
support API / projection contract 一致性已收口；该一致性已由后续 AgentSmith gate
切片完成。项目 pre-GA 不为旧输入、旧路径、旧命名保留长期心智负担；
...
DeepSeek endpoint/model，LLMUP real smoke 15 passed / 0 failed /
...
runner digest adoption 已完成，release-kit managed runner image closure consumption 和
AgentSmith support API / projection contract consistency focused gate 也已完成；
