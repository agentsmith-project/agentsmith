...
async function createFileLibrary(baseUrl: string, name = 'Artifact Workspace'): Promise<{ id: string }> {
...
describe('api-entry-node notebook task artifact routes', () => {
  it('deduplicates notebook task artifacts by task_relative_path across repeated execution artifact frames', async () => {
    ...
    const workspaceLibrary = await createFileLibrary(baseUrl, 'Artifact Dedupe Workspace');
      ...
      ws.on('message', (raw) => {
        ...
        if (msg.type === 'server.hello') {
          ws.send(JSON.stringify({
            type: 'agent.ready',
            payload: { capabilities: { wire_api: 'responses' } },
          }));
          markRunnerReady?.();
  ...
  it('downloads notebook task artifact content in local backend when inline content is available', async () => {
    const { baseUrl } = startServer();
    const workspaceLibrary = await createFileLibrary(baseUrl, 'Artifact Download Workspace');
