...
if demo_mode_is_full; then
  for required_nonempty_key in INTERNAL_AGENT_K8S_NAMESPACE INTERNAL_AGENT_JUICEFS_CSI_DRIVER INTERNAL_AGENT_WORKSPACE_CAPACITY INTERNAL_AGENT_JUICEFS_MOUNT_IMAGE; do
    if [[ -z "${!required_nonempty_key:-}" ]]; then
      die "missing required site.env value: ${required_nonempty_key}"
    fi
  done
