...
ensure_juicefs_csi() {
  ...
  if ! kubectl get csidriver "${CSI_DRIVER}" >/dev/null 2>&1; then
    internal_info "installing JuiceFS CSI driver ${CSI_DRIVER}"
  ...
  [[ -n "${csi_namespace}" ]] || csi_namespace="juicefs-system"
...
INTERNAL_AGENT_JUICEFS_STORAGE_ENDPOINT_OVERRIDE_VALUE="${INTERNAL_AGENT_JUICEFS_STORAGE_ENDPOINT_OVERRIDE}"
