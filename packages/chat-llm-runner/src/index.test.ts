...
describe('chat-llm-runner', () => {
  ...
  it('announces anthropic_messages support and roundtrips anthropic native chat requests', async () => {
    const server = http.createServer();
    servers.push(server);
    const wss = new WebSocketServer({ noServer: true });
    const upstreamServer = http.createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        expect(req.url).toContain('/workspaces/ws_test/projects/proj_test/endpoints/ep_test/proxy/anthropic/messages');
        expect(req.headers.authorization).toBe('Bearer exec_test');
        expect(body).toEqual({
        ...
        res.end(JSON.stringify({
          id: 'msg_1',
          type: 'message',
          content: [{ type: 'text', text: 'latest turn via anthropic' }],
    ...
    wss.on('connection', (ws) => {
      ...
      ws.once('message', (raw) => {
        ...
        ws.send(JSON.stringify({
          ...
          payload: {
            ...
            execution_context: {
              interaction_kind: 'chat',
              session_id: 'session_chat_anthropic',
              workspace_id: 'ws_test',
              project_id: 'proj_test',
              endpoint_id: 'ep_test',
              execution_ticket: 'exec_test',
      ...
      ws.on('message', (raw) => {
        const message = JSON.parse(raw.toString('utf-8')) as {
          type?: string;
          request_id?: string;
          payload?: { delta?: string } & Record<string, unknown>;
        };
        if (message.type === 'agent.response.delta' && message.payload?.delta) {
          echoedText += message.payload.delta;
        }
    ...
    await expect
      ...
      .toMatchObject({
        ...
        capabilities: {
          ...
          supported_wire_apis: ['chat', 'responses', 'anthropic_messages'],
        },
        request_details: {
    ...
    await expect
      ...
      .toBe('latest turn via anthropic');
  ...
  it('emits a reclaim warning when a continuation session workspace is missing', async () => {
    ...
    await expect
      .poll(() => warningPayload, { timeout: 30_000, interval: 250 })
      .toMatchObject({
        category: 'warning',
    ...
    await expect
      .poll(() => doneReceived, { timeout: 30_000, interval: 250 })
      .toBe(true);
  ...
  it('aborts the upstream request when the server cancels an in-flight chat request', async () => {
    const server = http.createServer();
    servers.push(server);
    const wss = new WebSocketServer({ noServer: true });
      ...
      ws.once('message', (raw) => {
        ...
        ws.send(JSON.stringify({
          ...
          payload: {
            ...
            execution_context: {
              interaction_kind: 'chat',
              session_id: 'session_chat_cancel',
              workspace_id: 'ws_test',
              project_id: 'proj_test',
              endpoint_id: 'ep_test',
              execution_ticket: 'exec_test',
    ...
    await expect
      .poll(() => upstreamStarted, { timeout: 5_000, interval: 10 })
      .toBe(true);

    cancelSent = true;
    const activeConnection = connection as WebSocket | null;
    if (!activeConnection) {
      throw new Error('chat_runner_test_connection_missing');
    }
