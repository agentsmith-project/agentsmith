...
FILE_LIBRARY_REAL_GATE_BIN_DIR="${FILE_LIBRARY_REAL_GATE_BIN_DIR:-${FILE_LIBRARY_REAL_GATE_RUNTIME_DIR}/bin}"
...
ensure_file_library_afscp_local_runtime() {
  ...
  export PATH="${FILE_LIBRARY_REAL_GATE_BIN_DIR}:${PATH}"
  export LD_LIBRARY_PATH="${FILE_LIBRARY_REAL_GATE_BIN_DIR}/juicefs-lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
  mkdir -p "${FILE_LIBRARY_REAL_GATE_BIN_DIR}"
  AFSCP_IMAGE="${AFSCP_LOCAL_RUNTIME_IMAGE:-${AFSCP_IMAGE:-}}" \
    AFSCP_JUICEFS_OUTPUT_PATH="${FILE_LIBRARY_REAL_GATE_BIN_DIR}/juicefs" \
