...
`)).toThrow(/asbcp_version.*v/u);
  ...
  it('generates a local-kind site env with immutable K8s digest refs and records host push refs', async () => {
    ...
    expect(result.evidence.images.afscp.source_ref).toContain('ghcr.io/agentsmith-project/agentsmith-fs-control-plane:v1.0.18@sha256:');
    expect(result.evidence.images.afscp.host_ref).toBe('localhost:5001/mbos/agentsmith-fs-control-plane:v1.0.18');
    expect(result.evidence.images.afscp.k8s_tag_ref).toBe('kind-registry:5000/mbos/agentsmith-fs-control-plane:v1.0.18');
  ...
  it('builds app and managed runner images, retags locked ASBCP/AFSCP/llmup images, and pushes local registry refs in order', async () => {
    ...
    expect(commandText).toContain('docker image inspect ghcr.io/agentsmith-project/agentsmith-fs-control-plane:v1.0.18@sha256:');
    expect(commandText).toContain('docker tag ghcr.io/agentsmith-project/agentsmith-fs-control-plane:v1.0.18@sha256:');
    expect(commandText).toContain('localhost:5001/mbos/agentsmith-fs-control-plane:v1.0.18');
    ...
    expect(commandText).toContain('docker push localhost:5001/mbos/agentsmith-fs-control-plane:v1.0.18');
