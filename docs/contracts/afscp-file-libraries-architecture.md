...
### Live File Tree and Workspace Binding Convergence

这里的 convergence 指 Files API、task file attachment、AFSCP workspace binding 在读、释放、restore/save point 控制操作中的工程收敛，不是用户可见的新产品流程。默认 browse/list/download 的当前工作树真相是 AFSCP repo payload root / managed runner HOME workspace tree。

| Surface | `pending` | `releasing` | `offline` | `not_found` |
| --- | --- | --- | --- | --- |
| Files | Return typed `file_library_list_pending` / read-export pending from the live read/export owner; do not return stale `200 items: []`. | Keep browse/list/download on the live Files read/export path; runtime release is not a default list precondition. | Treat as no active writer for Files reads and use only the Files read/export path; do not create an executable connector. | Treat as no active writer for Files reads; do not synthesize an executable connector. |
...
| AFSCP workspace binding | Keep the owning operation pending until terminal success or typed failure/blocker. | Keep explicit release/restore convergence through the workspace binding owner until terminal `released`, `revoked`, `expired`, or `deleted`. | Treat as no active holder for Files reads; executable reattachment must use the Agent Task sandbox owner path. | Treat as no active holder for Files reads; executable reattachment must use the Agent Task sandbox owner path. |
| Read export | Return typed pending when live read/export/projection is not ready. | Keep existing live read/export semantics; explicit release fences do not make browse/list/download pretend success. | Create or reuse the live read/export path without inventing an executable attachment. | Create or reuse the live read/export path if available; otherwise return typed pending/unavailable. |

- live file tree: File Library current working tree is the AFSCP repo payload root / managed runner HOME workspace tree. Files written to `$HOME/workspace/...` and `$HOME/workspace/.artifacts/...` must be visible to Files list/download through the same payload-root fact.
- read export `pending` / `file_library_list_pending`: the list API returns typed pending from the live read/export owner. It must not start runtime-access release as a side effect and must not return stale `200 items: []` when real files exist.
- entries list/download with an active or idle runtime writer: call the Files live read/export path directly. Do not call `releaseRuntimeAccessForFileLibrary` before browse/list/download.
- save point/restore: either snapshot/restore the live payload root or return typed pending/unavailable while writer quiesce, projection, or owner readiness is still unresolved. Do not report stale empty success.
- workspace binding `releasing` / `release_pending`: explicit runtime-access release and restore-related control operations keep using bounded workspace-binding owner convergence until a terminal release state or a typed blocker is returned.
...
- workspace binding revoke idempotency conflict: AFSCP 409 `conflict` during runtime release is a retryable readiness conflict for explicit release/restore control paths; repeated focused failures with the same evidence are stability blockers rather than one-off Files bugs.
...
- Product Readiness evidence must preserve the Files restore continuation focused backend-real gate before the full Product Readiness campaign, because this slice proves restore can continue through pending read/export and explicit release convergence.
