...
| files | manage Version & templates: restore points, direct restore operation, task file template publish/unpublish/delete, and read redacted file-library operation projections | `project:files:update` | file-library save point/restore/template routes, `GET /file-library-operations/{operationId}` | Version & templates entry point hidden or disabled |
