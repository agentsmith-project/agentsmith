...
main() {
  ...
  denied_code="$(
    curl -sS -o "${denied_file}" -w '%{http_code}' \
      ...
      -H "Authorization: Bearer ${member_token}" \
      -H "Content-Type: application/json" \
      --data "{\"name\":\"${smoke_name}\",\"permissions\":[\"project:member:view\"]}" || true
  )"
  ...
  info "granting project:member:manage to member"
  ...
  patch_grant_code="$(
    curl -sS -o "${patch_file}" -w '%{http_code}' \
      ...
      -H "Authorization: Bearer ${owner_token}" \
      -H "Content-Type: application/json" \
      --data '{"mode":"custom","permissions":["project:member:manage"]}' || true
  )"
