...
if ! command -v curl >/dev/null 2>&1; then
  echo "[runtime-stream-bench] curl is required" >&2
  exit 1
fi

if ! command -v node >/dev/null 2>&1; then
  echo "[runtime-stream-bench] node is required" >&2
  exit 1
fi

if [[ ! -f "${TOKEN_FILE}" ]]; then
  echo "[runtime-stream-bench] token file not found: ${TOKEN_FILE}" >&2
  exit 1
fi

TOKEN="$(tr -d '\r\n' < "${TOKEN_FILE}")"
if [[ -z "${TOKEN}" ]]; then
  echo "[runtime-stream-bench] token is empty: ${TOKEN_FILE}" >&2
  exit 1
fi

TMP_DIR="$(mktemp -d /tmp/agentsmith-runtime-stream-bench.XXXXXX)"
...
echo "[runtime-stream-bench] start requests=${REQUESTS} concurrency=${CONCURRENCY} model=${MODEL}"
...
const summary = {
  kind: "runtime_streaming_benchmark",
...
cat "${SUMMARY_JSON}"
echo
echo "[runtime-stream-bench] artifacts_dir=${TMP_DIR}"
...
if [[ "${STRICT_GATE}" == "1" ]]; then
  ...
  if awk "BEGIN { exit !(${P95} <= ${THRESHOLD_P95_MS}) }"; then
    ...
    echo "[runtime-stream-bench] gate failed: p95=${P95} > ${THRESHOLD_P95_MS}" >&2
    exit 1
  fi
  if awk "BEGIN { exit !(${ERROR_RATE} <= ${THRESHOLD_ERROR_RATE}) }"; then
    :
  else
    echo "[runtime-stream-bench] gate failed: error_rate=${ERROR_RATE} > ${THRESHOLD_ERROR_RATE}" >&2
    exit 1
  fi
fi
