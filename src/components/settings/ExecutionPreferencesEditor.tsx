...
export interface ExecutionPreferencesEditorProps {
  value: ExecutionPreferences;
  onChange: (value: ExecutionPreferences) => void;
  disabled?: boolean;
}

export function ExecutionPreferencesEditor({
  value,
  onChange,
  disabled,
}: ExecutionPreferencesEditorProps) {
  ...
  const handleFormChange = (path: string[], newVal: unknown) => {
    ...
    onChange(next as ExecutionPreferences);
  };

  const handleJsonApply = () => {
    try {
      const parsed = JSON.parse(jsonText) as ExecutionPreferences;
      onChange(parsed);
      setJsonError(null);
      setViewMode('form');
    } catch (e) {
