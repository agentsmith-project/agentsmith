...
- 按**功能/页面**分块，每块一个**顶级 namespace**，例如：`common`、`nav`、`auth`、`projects`、`files`、`members`、`notebook`、`chat`、`audit`、`usage` 等（与现有文件一致）。  
  对外产品文案统一使用 **Notebook**，i18n namespace 使用 `notebook`。

### 2.2 命名与复用

- **同一意思同一 key**：全项目同一句文案只对应一个 key，多处复用该 key，不要重复造 key。
- **通用文案**：按钮、状态、提示等通用语放在 `common`（如：`common.loading`、`common.save`、`common.cancel`）。
- **页面/模块专属**：该页面/模块才用到的文案放在对应 namespace（如 `projects.title`、`files.upload`）。
- **带占位符**：用 `{name}` 形式，例如：`"count": "All Projects ({count})"`，中英文都保留相同占位符。

### 2.3 中文翻译要求

- 使用**简体中文**，用语符合产品/后台管理场景。
- 专有名词（如 Keycloak、AIReady、Notebook）可保留英文，其余尽量翻译。
...
3. 用 `/zh-CN/...` 和 `/en-US/...` 对比，**列出仍为英文的页面/区块**（例如：登录、工作空间选择、项目列表、概览、Chat、Notebook、Files、Agents、Endpoints、Members、Audit、Usage、Settings 等）。
4. 在代码里**搜索硬编码英文字符串**（如 `"Create"`、`"Loading..."`、`Select Workspace`），定位到具体文件和行号，做成清单。

### Step 2：规划 namespace 和 key

1. 打开 `src/messages/en-US.json`，看现有顶级 key：`common`、`nav`、`auth`、`workspace`、`project`、`files`、`members`、`notebook`、`chat`、`audit`、`usage`、`projects` 等。
2. 对每个“未翻译”的页面/模块，决定：
...
| 聊天                | `chat`             | `chat/` 下页面与组件                       |
| Notebook / Task    | `notebook` | `notebook/` 下页面与组件 |
| 文件 / Files      | `files`    | `files/` 下页面与组件 |
