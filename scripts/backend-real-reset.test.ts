...
describe('backend-real reset Kubernetes safety guard', () => {
  ...
  it('clears local backend-real state without sidecar cleanup messaging', () => {
    ...
    writeFileSync(path.join(runtimeDir, 'manager.pid'), '12345\n');
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(`clearing backend-real state under ${stateDir}`);
    expect(result.stdout).not.toContain('stopping backend-real sandbox');
