...
if [[ -z "${TASK_ID}" && "${PREPARE_TASK}" == "1" ]]; then
  ...
  for i in $(seq 1 "${TURNS}"); do
    if [[ "${i}" == "1" ]]; then
      (
        cd "${ROOT_DIR}"
        PROMPT="${TURN_PROMPT_PREFIX} ${i}" make agent-task-smoke-task >/dev/null
      )
      TASK_ID="$(state_get task.last_id)"
    else
      TOKEN="$(cat "${TOKEN_FILE}")"
      BASE="${API_BASE%/}/api/v1/workspaces/${WORKSPACE_ID}/projects/${PROJECT_ID}"
      curl -sS -X POST "${BASE}/tasks/${TASK_ID}/runs" \
        -H "Authorization: Bearer ${TOKEN}" \
        -H 'Content-Type: application/json' \
        -d "$(node -e 'console.log(JSON.stringify({intent:process.argv[1]}))' "${TURN_PROMPT_PREFIX} ${i}")" >/dev/null
