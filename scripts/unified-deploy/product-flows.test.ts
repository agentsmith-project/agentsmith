...
function makeFocusedAgentTaskFetch(observed: {
  ...
  return vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.includes('/audit') && method === 'GET' && observed.auditItems) {
      return responseJson(200, {
        items: observed.auditItems,
        total: observed.auditItems.length,
      });
    }
    if (url.includes('/usage/facts') && method === 'GET' && observed.usageItems) {
      return responseJson(200, {
        items: observed.usageItems,
        total: observed.usageItems.length,
      });
    }
    return responseJson(404, { error: `unexpected:${method}:${url}` });
  });
...
describe('unified deploy product flow producer', () => {
  ...
  it('fails audit and usage readback when same-window records were not generated by this smoke', async () => {
    const observed = {
      chatRequests: 0,
      taskCreatePayloads: [] as Record<string, unknown>[],
      auditItems: [
        {
          id: 'audit_other_endpoint',
          project_id: 'proj_focused',
          action: 'endpoint.create',
          resource_type: 'endpoint',
          resource_id: 'ep_other',
          request_id: 'unrelated-request',
        },
      ],
      usageItems: [
        {
          id: 'usage_other_endpoint',
          project_id: 'proj_focused',
          resource_type: 'endpoint',
          resource_id: 'ep_other',
          request_id: 'unrelated-request',
    ...
    vi.useFakeTimers();
    try {
      const resultPromise = runUnifiedDeployProductFlowsProducer({
        siteEnvPath: 'site.env',
        substrateTruthPath: 'connection.env',
        evidenceDir: 'evidence',
        fs: makeFs(),
        fetch: makeFocusedAgentTaskFetch(observed),
        flowIds: ['workspace_project', 'chat_via_llmup', 'files', 'agent_task_managed_runner', 'audit', 'usage'],
        backendBootstrapper: async () => ({}),
        keycloakBootstrapper: async () => ({
          users: {
            devAdmin: { user_id: 'kc-dev-admin', email: 'dev-admin@example.com', name: 'Dev Admin' },
      ...
      const auditFlow = result.evidence.flows.find((flow) => flow.flow === 'audit');
      const usageFlow = result.evidence.flows.find((flow) => flow.flow === 'usage');
      expect(result.status).toBe('failed');
      expect(auditFlow?.status).toBe('failed');
      expect(auditFlow?.failure?.message).toContain('generated by this smoke');
      expect(usageFlow?.status).toBe('failed');
      expect(usageFlow?.failure?.message).toContain('generated by this smoke');
    } finally {
      vi.useRealTimers();
    }
  });

  it('passes audit and usage readback when records bind to a smoke-created resource', async () => {
    const observed = {
      chatRequests: 0,
      auditItems: [
        {
          id: 'audit_endpoint_create',
          project_id: 'proj_focused',
          action: 'endpoint.create',
          resource_type: 'endpoint',
          resource_id: 'ep_focused',
          request_id: 'audit-system-request',
        },
      ],
      usageItems: [
        {
          ...
          resource_type: 'endpoint',
          resource_id: 'ep_focused',
          request_id: 'usage-system-request',
        },
      ],
    };
    ...
    const result = await runUnifiedDeployProductFlowsProducer({
      ...
      fs: makeFs(),
      fetch: makeFocusedAgentTaskFetch(observed),
      flowIds: ['workspace_project', 'chat_via_llmup', 'audit', 'usage'],
      backendBootstrapper: async () => ({}),
      keycloakBootstrapper: async () => ({
        users: {
          devAdmin: { user_id: 'kc-dev-admin', email: 'dev-admin@example.com', name: 'Dev Admin' },
    ...
    const usageFlow = result.evidence.flows.find((flow) => flow.flow === 'usage');
    expect(result.status).toBe('passed');
    expect(auditFlow?.checks).toMatchObject({
      matched_actions: ['endpoint.create'],
      matched_resource_ids: ['ep_focused'],
    });
    expect(usageFlow?.checks).toMatchObject({
      matched_fact_ids: ['usage_endpoint'],
      matched_resource_ids: ['ep_focused'],
      matched_resource_types: ['endpoint'],
    });
  });
