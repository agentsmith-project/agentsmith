...
function shouldUseBwrap(mode: RunnerMode): boolean {
  return mode === 'host_external' || mode === 'docker_external';
}

function isStrictBwrapRequired(mode: RunnerMode): boolean {
  if (mode === 'docker_external') return true;
  const raw = (process.env.MBOS_EXTERNAL_REQUIRE_BWRAP ?? '').trim().toLowerCase();
