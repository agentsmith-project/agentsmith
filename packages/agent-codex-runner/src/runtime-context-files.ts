...
export type RuntimeContextFileItem = {
...
function normalizeRelativePath(input: string): string {
  ...
  if (!trimmed) {
    throw new Error('runtime_file_path_missing');
  }
  if (trimmed.startsWith('/') || /^[A-Za-z]:[\\/]/.test(trimmed)) {
    throw new Error('runtime_file_path_must_be_relative');
  }
  const normalized = normalize(trimmed).replace(/\\/g, '/');
  if (!normalized || normalized.startsWith('../') || normalized === '..') {
    throw new Error('runtime_file_path_outside_workspace');
  }
  if (normalized.includes('/../') || normalized.includes('/./') || normalized.endsWith('/..')) {
    throw new Error('runtime_file_path_invalid_segments');
  }
  return normalized;
}
...
function resolveSafeTargetPath(cwd: string, relativePath: string): string {
  ...
  if (rel.startsWith('..') || rel.includes('/..') || rel.includes('\\..')) {
    throw new Error('runtime_file_path_escape_detected');
  }
  return targetPath;
}

export async function applyRuntimeContextFiles(
  cwd: string,
  inputItems: RuntimeContextFileItem[] | undefined,
): Promise<{ written: number; totalBytes: number }> {
  const items = Array.isArray(inputItems) ? inputItems : [];
  if (items.length > MAX_FILE_COUNT) {
    throw new Error('runtime_files_count_exceeded');
  }
  ...
  for (const item of items) {
    ...
    if (!relativePath) {
      throw new Error('runtime_file_relative_path_required');
    }
    const bytes = Buffer.byteLength(content, 'utf-8');
    if (bytes > MAX_FILE_SIZE_BYTES) {
      throw new Error('runtime_file_size_exceeded');
    }
    totalBytes += bytes;
    if (totalBytes > MAX_TOTAL_SIZE_BYTES) {
      throw new Error('runtime_files_total_size_exceeded');
    }
