...
describe('createReleaseGateRunner', () => {
  it('triggers a full run and records completion', async () => {
    const runsDir = mkdtempSync(join(tmpdir(), 'release-gate-runner-'));
    let executeCalls = 0;
    const runner = createReleaseGateRunner({
      releaseRunsDir: runsDir,
      executeReleaseReport: async () => {
        executeCalls += 1;
      },
    });
  ...
  it('reruns only failed checks from a prior run', async () => {
    ...
    writeFileSync(join(runsDir, 'sample-run.json'), JSON.stringify({
      ...
      passed_checks: 5,
      failed_checks: 1,
      failed_step_names: ['Governance release smoke'],
    ...
    const runner = createReleaseGateRunner({
      releaseRunsDir: runsDir,
      executeReleaseReport: async (args) => {
        capturedArgs = args;
      },
    });
  ...
  it('rejects concurrent runs with an explicit busy error', async () => {
    ...
    ).rejects.toThrow('release_gate_runner_busy');
