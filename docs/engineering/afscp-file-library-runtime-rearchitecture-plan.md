...
Primary input: use `ghcr.io/agentsmith-project/agentsmith-fs-control-plane:v1.0.0` for File Library storage runtime, with AgentSmith as the product authority for workspace/project, permissions, file library catalog, task binding, task file template availability, UX, and audit projection.
...
AFSCP v1.0.0 does not expose a first-class file browse/upload/download/move API. It exposes export sessions and a WebDAV gateway contract.
...
- It must not receive AFSCP root credentials, raw control paths, Secret refs,
  ...
  material.
- If AFSCP v1.0.0 cannot safely express this export-backed developer runner
...
13. Developer runner parity is a target of this milestone and requires an AFSCP
    ...
    heartbeat/reconcile, and export/session accounting. If AFSCP v1.0.0 cannot
...
Slice 5 is a milestone target. If AFSCP v1.0.0 cannot provide the safe
...
- if AFSCP v1.0.0 cannot support the safe lease/connector abstraction, stop Slice 5 and open the upstream AFSCP contract change; do not add a dedicated developer-orchestrator identity or raw storage workaround.
...
this milestone, but if AFSCP v1.0.0 officially blocks the safe export-backed
