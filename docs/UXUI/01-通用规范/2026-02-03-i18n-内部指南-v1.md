...
- 按**功能/页面**分块，每块一个**顶级 namespace**，例如：`common`、`nav`、`auth`、`projects`、`sources`、`members`、`studio`、`chat`、`audit`、`usage` 等（与现有文件一致）。  
  注：对外产品文案统一使用 **AI Studio**，`studio` 仅为历史 namespace 兼容名。
...
- 专有名词（如 Keycloak、AIReady、AI Studio）可保留英文，其余尽量翻译。
...
3. 用 `/zh-CN/...` 和 `/en-US/...` 对比，**列出仍为英文的页面/区块**（例如：登录、工作空间选择、项目列表、概览、Chat、AI Studio、Sources、Agents、Endpoints、Members、Audit、Usage、Settings 等）。
4. 在代码里**搜索硬编码英文字符串**（如 `"Create"`、`"Loading..."`、`Select Workspace`），定位到具体文件和行号，做成清单。

### Step 2：规划 namespace 和 key

1. 打开 `src/messages/en-US.json`，看现有顶级 key：`common`、`nav`、`auth`、`workspace`、`project`、`sources`、`members`、`studio`、`chat`、`audit`、`usage`、`projects` 等。
2. 对每个“未翻译”的页面/模块，决定：
...
| 聊天                | `chat`             | `chat/` 下页面与组件                       |
| AI Studio / Task    | `studio`（兼容） | `studio/` 下页面与组件（namespace 暂不改名，对外名称统一为 AI Studio） |
