...
describe('ArtifactCard', () => {
  ...
  describe('Text Artifact Rendering', () => {
    ...
    it('reveals the floating detail panel on hover', () => {
      ...
      const card = screen.getByTestId('agent-tasks__artifact-card');
      fireEvent.mouseEnter(card);
      expect(screen.getByTestId('agent-tasks__artifact-hover-panel')).toBeInTheDocument();
      expect(screen.getByText(/This is a text artifact/)).toBeInTheDocument();
      fireEvent.mouseLeave(card);
      act(() => {
        vi.advanceTimersByTime(140);
      });
      expect(screen.queryByTestId('agent-tasks__artifact-hover-panel')).not.toBeInTheDocument();
    });
    ...
    it('keeps the hover panel open long enough to move into it', () => {
      ...
      const card = screen.getByTestId('agent-tasks__artifact-card');
      fireEvent.mouseEnter(card);
      const panel = screen.getByTestId('agent-tasks__artifact-hover-panel');
      fireEvent.mouseLeave(card);
    ...
    it('shows copy button for text artifacts', () => {
      renderComponent(mockTextArtifact);
      fireEvent.mouseEnter(screen.getByTestId('agent-tasks__artifact-card'));
  ...
  describe('Layout and Styling', () => {
    ...
    it('uses flex layout for actions', () => {
      ...
      fireEvent.mouseEnter(screen.getByTestId('agent-tasks__artifact-card'));
      const actions = screen.getByTestId('agent-tasks__artifact-hover-panel').querySelector('.grid');
