...
const COLLECTION = 'release_incident_history';

export type ReleaseIncidentHistoryRecord = {
  id: string;
  incident_id: string;
...
export async function listReleaseIncidentHistory(
  docStore: JsonDocStorePort,
  params: {
    incidentId?: string;
    escalationId?: string;
  } = {},
): Promise<ReleaseIncidentHistoryRecord[]> {
  ...
  const items = await docStore.list<ReleaseIncidentHistoryRecord>(COLLECTION, Object.keys(query).length > 0 ? query : undefined);
  return items.sort((a, b) => b.created_at.localeCompare(a.created_at));
}

export async function recordReleaseEscalationAssignmentHistory(
  docStore: JsonDocStorePort,
  params: {
    incidentId: string;
    escalationId: string;
  ...
  const record: ReleaseIncidentHistoryRecord = {
    id: `rih_${randomUUID()}`,
