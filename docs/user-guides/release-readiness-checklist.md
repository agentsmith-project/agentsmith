...
- release-kit handoff boundary: AgentSmith product:ready is product readiness / local complete / current product gate for product evidence, full visual, backend-real product readiness, and terminal aggregate evidence for handoff input completeness. It is not a deployment/package/operator verdict. Unified deploy and local-kind deploy commands are transition-only focused diagnostics / 过渡期专项诊断. The product-flow lane is only the AgentSmith-owned producer for canonical post-deploy product smoke reports consumed by the release-kit final GA facade (`bash scripts/operator-release.sh --ga-report`); release-kit final GA requires at least one online report and one airgap report. It is not a default product readiness or release-full step. Release-kit owns the final GA verdict through repo-local gate/evidence over deployment/package/operator inputs; AgentSmith retains product readiness, images/release contract, local full test, and thin adapter.
- release-kit final GA 的 `use_existing` evidence 可以来自 local actual deployment with operator-provided existing substrates，作为当前 release 的 path-level input；必须绑定当前 release inputs、当前 substrate truth digest、deployment path report 和 smoke，旧 13f 或其他历史 evidence 不可复用。云托管服务只是常见实施配置，不是当前 GA 必过 gate；`use_existing` 不运行 substrate installer，也不得消费 `install_substrates` installer report 或 namespace-scoped installer evidence。
...
| post-deploy product smoke handoff producer | release-kit GA handoff artifact | 绑定 downloaded `agentsmith-release-contract.json`、同一 deployed target 的 site env 和 neutral `agentsmith.substrate-connection.truth/v1` substrate truth JSON，按 deployed target/run 输出 canonical report；runtime-only substrate env projection 只通过 request env 或显式 path 供 product-flow 执行，不写入 artifact；Docker substrate env 不可作为 GA handoff truth；final GA 至少把 `<online-ga-smoke-evidence-root>/post-deploy-product-smoke/post-deploy-product-smoke-report.json` 和 `<airgap-ga-smoke-evidence-root>/post-deploy-product-smoke/post-deploy-product-smoke-report.json` 作为 repeated `--post-deploy-product-smoke-report` inputs 交给 `bash scripts/operator-release.sh --ga-report`；不属于默认 `product:ready` / release-full |
...
5. Post-deploy product smoke handoff 只消费 downloaded `agentsmith-release-contract.json`、同一 deployed target 的 site env / neutral substrate connection truth JSON 与该目标的 `<ga-smoke-evidence-root>`；product-flow 执行所需 runtime substrate env projection 只走 request env 或显式 path，不进入 finalized report/artifact；final GA 至少需要 online 和 airgap 两个目标各一份 report；不要把该 lane 加回默认 `product:ready` / release-full。
