    ...
    case "${code}" in
      ...
      401)
        if [[ "${refreshed}" == "0" ]]; then
          ...
          (cd "${ROOT_DIR}" && BASE_URL="${BASE_URL:-http://localhost:3001}" make agent-runner-refresh-token)
          refreshed=1
          continue
        fi
        ;;
...
run_basic_smoke() {
  info "running agent-task-smoke-task"
...
run_matplotlib_smoke() {
  ...
  if [[ -z "${token}" || -z "${project_id}" ]]; then
    err "missing token/project metadata in $(backend_real_state_file); run bootstrap/init-resources first"
    return 1
  fi
  local task_id
  task_id="$(
    curl -sS -X POST "${api_base}/workspaces/${ws}/projects/${project_id}/tasks" \
      -H "Authorization: Bearer ${token}" \
      -H 'Content-Type: application/json' \
      --data "{\"title\":\"engineering-mpl-smoke-$(date +%s)\"}" | \
      sed -nE 's/.*"id":"([^"]+)".*/\1/p' | head -n1
  )"
  [[ -n "${task_id}" ]] || { err "failed to create task"; return 1; }
  info "task_id=${task_id}"
  curl -sS -X POST "${api_base}/workspaces/${ws}/projects/${project_id}/tasks/${task_id}/runs" \
    -H "Authorization: Bearer ${token}" \
    -H 'Content-Type: application/json' \
    --data '{"intent":"Use python and matplotlib to generate a simple plot, save it to ./artifacts/plot.png, do not call plt.show(), then reply with the saved filename only."}' >/dev/null
