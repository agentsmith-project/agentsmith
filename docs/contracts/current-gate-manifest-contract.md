...
- the product-flow lane requires a release contract from `UNIFIED_DEPLOY_RELEASE_CONTRACT` or `AGENTSMITH_RELEASE_CONTRACT_PATH`; `UNIFIED_DEPLOY_RELEASE_SITE_ENV` selects the deployed target site env, `UNIFIED_DEPLOY_RELEASE_SUBSTRATE_TRUTH` selects the same deployed target neutral `agentsmith.substrate-connection.truth/v1` JSON, runtime-only substrate env projection is supplied separately for product-flow execution, and `UNIFIED_DEPLOY_RELEASE_ROOT_DIR` selects the smoke evidence root
