...
- AFSCP workspace binding `offline` / `not_found`: treat as no active writer for Files read export; any new executable attachment must be created by the Agent Task sandbox owner path, not by Files inventing a connector.
...
- Files entries list with an idle runtime writer: converge runtime-access release before creating a read export; while release is still pending, return typed `file_library_list_pending` instead of reading a stale pre-release projection.
- Files runtime release with AFSCP revoke idempotency conflict: map the AFSCP 409 `conflict` evidence to retryable runtime readiness so read export/listing stays pending instead of returning a stale or failed Files projection.
