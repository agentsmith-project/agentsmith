...
export const CURRENT_CI_WORKFLOW_MANIFEST: readonly CurrentCIWorkflowDefinition[] = [
  ...
  defineCurrentCIWorkflow({
    path: '.github/workflows/product-readiness-artifact.yml',
    workflowName: 'Product Readiness Artifact',
    role: 'release_artifact_producer',
    ...
    jobs: [
      {
        ...
        requiresSecrets: true,
        evidenceRequired: true,
        evidenceFamilies: ['product_readiness_report', 'test_results', 'playwright_report'],
        artifactPaths: [
        ...
        scheduled: false,
        productReadinessBlocking: false,
        notes: 'Runs AgentSmith product readiness with a downloaded release contract and always uploads staged campaign evidence plus test reports, excluding runtime caches; the final GA handoff report remains a success-only file check, not a failed-run verdict.',
      },
    ],
  }),
  defineCurrentCIWorkflow({
    path: '.github/workflows/post-deploy-product-smoke-artifact.yml',
    workflowName: 'Post-deploy Product Smoke Artifact',
    role: 'release_artifact_producer',
    triggers: ['workflow_dispatch'],
    blockingFor: ['manual', 'handoff'],
    scheduled: false,
