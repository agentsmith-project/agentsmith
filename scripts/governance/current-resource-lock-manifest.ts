...
export const CURRENT_RESOURCE_LOCK_MANIFEST: readonly CurrentResourceLockDefinition[] = [
  ...
  },
  {
    ...
    scope: 'local_host',
    mode: 'exclusive',
    reason: 'Local backend, browser, and rehearsal substrates compete for the same host services and should not be treated as independent worlds.',
    owners: {
      gateIds: [
      ...
      commandSurfaces: ['backend-real stack', 'local rehearsal substrate'],
    },
    appliesTo: {
      gateIds: [
        'test-backend-real-core',
      ...
      runtimeLines: ['local-manual', 'demo-rehearsal', 'cluster-rehearsal'],
      paths: [
  ...
  },
  {
    ...
    scope: 'local_host',
    mode: 'exclusive',
    reason: 'Reset, down, cleanup, and destructive rehearsal lifecycle commands can invalidate another active local run.',
    owners: {
      gateIds: ['lane-demo-rehearsal', 'lane-cluster-rehearsal', 'lane-backend-real-release'],
      npmScripts: [
        'integration:deps:down',
    ...
    appliesTo: {
      ...
      runtimeLines: ['local-manual', 'demo-rehearsal', 'cluster-rehearsal'],
      paths: [
        'scripts/scenarios/<scenario-id>/reset.sh',
        'scripts/backend-real-reset.sh',
      ],
    },
    enforcement: 'existing_preflight_cleanup',
  ...
  },
  {
    ...
    scope: 'local_host',
    mode: 'exclusive',
    reason: 'The dev API, Next app, local registries, and scenario sandboxes use fixed host ports that cannot be shared by concurrent runs.',
    owners: {
      gateIds: [
        'gate-default',
        'lane-visual',
      ...
      commandSurfaces: ['local dev services', 'scenario sandbox services'],
    },
    appliesTo: {
      gateIds: [
        'gate-default',
      ...
      ports: [
        ...
        },
        {
          ...
          values: [29280, 29080],
          label: 'demo and cluster rehearsal sandbox host ports',
        },
        {
          kind: 'family',
          name: 'scenario-local-registry-host-ports',
          values: [5002, 5003],
          label: 'scenario-owned local registry host ports',
        },
      ],
    },
    enforcement: 'modeled_only',
  },
  {
    ...
    appliesTo: {
      ...
      runtimeLines: ['local-manual', 'demo-rehearsal', 'cluster-rehearsal', 'demo-deploy', 'cluster-deploy'],
      paths: [
  ...
  },
  {
    ...
    scope: 'local_host',
    mode: 'exclusive',
    reason: 'Demo and cluster rehearsals each own a local kind world, registry identity, and report tree instead of sharing a generic scenario world.',
    owners: {
      gateIds: ['lane-demo-rehearsal', 'lane-cluster-rehearsal'],
      npmScripts: ['lane:demo-rehearsal', 'lane:cluster-rehearsal', 'rehearse:demo', 'rehearse:cluster'],
      commandSurfaces: ['demo rehearsal world', 'cluster rehearsal world'],
    },
    appliesTo: {
      gateIds: ['lane-demo-rehearsal', 'lane-cluster-rehearsal'],
      npmScripts: ['lane:demo-rehearsal', 'lane:cluster-rehearsal', 'rehearse:demo', 'rehearse:cluster'],
      runtimeLines: ['demo-rehearsal', 'cluster-rehearsal'],
      paths: [
        'scripts/scenarios/<scenario-id>/**',
        'artifacts/release-runs/<campaign-run-id>/lane-<scenario-id>-rehearsal/**',
      ],
      ports: [
        {
          kind: 'family',
