...
Status: `p1_1_artifact_producer_passed_p2_online_target_registry_apply_evidence_spine_done_p2_registry_presence_focused_done_p2_operator_preloaded_registry_prereq_binding_done_operator_signoff_intake_guard_done_p3_airgap_bundle_render_check_done_p3_app_current_inventory_closure_done_p3_airgap_image_archive_materiality_focused_done_p3_airgap_image_load_import_focused_done_p3_substrate_pack_focused_done_p5_1_start_guard_done_p5_2_formal_artifact_handoff_done_p5_3a_runner_release_manifest_skeleton_done_agentsmith_p5_3a_machine_contract_alignment_done_p5_3b_runner_runtime_fast_first_half_done_p5_3b_projection_only_boundary_fix_done_p5_3b_runner_boundary_closure_done_p5_runner_image_smoke_done_p5_runner_publish_manifest_evidence_done_agentsmith_manifest_lock_adoption_done_release_contract_runner_digest_adoption_done_release_kit_managed_runner_image_closure_consumption_done_agentsmith_support_api_projection_consistency_done_p5_request_scoped_projected_dependencies_contract_env_wiring_done`
Date: 2026-05-27
...
该切片不是 runner runtime/backend-real/task execution/Codex smoke 完成，不是
release readiness，不是 deployment/offline/airgap readiness，也不是 AgentSmith full
adoption。
...
dependencies contract/env wiring focused slice 也已完成。release-kit 已消费
...
runtime/backend-real/task execution/Codex smoke、AgentSmith full adoption、
...
### P2 online spine / registry prerequisite binding / operator signoff intake / P3 focused diagnostics/inventory closure / P5.3a manifest skeleton / P5.3b boundary closure / P5 image smoke / P5 publish evidence / release-kit image closure consumption / P5 request projection env wiring 完成后的后续门禁
...
1. P2 online target-registry confirmed apply/evidence spine、
   ...
   P5 request-scoped projected dependencies contract/env wiring 已有完成证据，
   但只解锁后续专项；
...
8. P5.3a/P5.3b first half/boundary closure/P5 image smoke/P5 publish manifest
   ...
   closure consumption 和 P5 request-scoped projected dependencies contract/env wiring
   也已完成。runtime source 迁出、no-push image smoke、focused publish
   evidence、该 consumption 和 projected dependency env wiring 都不是 release readiness；
   后续仍需按 KISS 小切片推进
...
34. 有没有把 P5 request-scoped projected dependencies contract/env wiring 写成 runner runtime/backend-real/task execution/Codex smoke 完成、release readiness、deployment/offline/airgap readiness、AgentSmith full adoption，或让 runner repo 解释 Context Store / managed credential / scope / write policy 语义、接受旧字段作为 legacy 成功路径？
...
final publish run `26471533836` success，manifest artifact id `7224661359`；P5
request-scoped projected dependencies contract/env wiring 已完成，AgentSmith commit
`8c6df24c` remote runs `26522251350` / `26522249787` / `26522250713` success，
runner commit `c67e837` remote run `26522674596` success。
...
P5 request-scoped projected dependencies contract/env wiring 也不是
runner runtime/backend-real/task execution/Codex smoke 完成、release readiness、
deployment/offline/airgap readiness 或 AgentSmith full adoption；runner repo 只消费
opaque request projection，AgentSmith/support API/contract 仍拥有投影语义；旧字段/旧职责继续
fail fast，不作为 legacy 成功路径。但仍不能宣称最终发布包闭环完成；monorepo runner build/dev 路径删除或归位仍受
...
P5 request-scoped projected dependencies contract/env wiring focused slice 已完成：
AgentSmith commit `8c6df24c` 已推送 main，remote Contracts Check run
`26522251350`、Image Publish run `26522249787`、Quality Gates run `26522250713`
success；runner commit `c67e837` 已推送 main，runner CI run `26522674596`
success。runner repo 只消费 `MBOS_AGENT_PROJECTED_DEPENDENCIES` opaque bulk env，
不新增 per-dependency env，不定义 Context Store / managed credential / scope /
write policy 语义；旧字段/旧职责继续 fail fast，不作为 legacy 成功路径。
下一步仍不是 release readiness。旧
`@mbos/agent-runner` shim、旧 env helper / monorepo-side `buildAgentRuntimeEnv` 和源码路径 runner image 都不能
