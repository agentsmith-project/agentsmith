...
wait_for_agent_online() {
  ...
  for i in $(seq 1 "${WAIT_AGENT_ONLINE_MAX}"); do
    ...
    if [[ "${diag_presence}" == "online" && -n "${diag_connected_at}" ]]; then
      echo "[load] agent online (presence=${diag_presence})"
      return 0
    fi
    sleep "${WAIT_AGENT_ONLINE_INTERVAL_SEC}"
  done
  echo "[load] ERROR: agent not online before timeout" >&2
  return 1
}
...
run_one() {
  ...
  create_task_resp="$(curl -sS -X POST "${BASE}/tasks" \
    ...
    -d "{\"title\":\"load-${idx}-$(date +%s)\",\"agent_id\":\"${AGENT_ID}\"}")" || {
  ...
  local post_code
  post_code="$(
    curl -sS -o /dev/null -w '%{http_code}' -X POST "${BASE}/tasks/${task_id}/messages" \
      -H "Authorization: Bearer ${TOKEN}" \
      -H 'Content-Type: application/json' \
      -d "$(node -e 'console.log(JSON.stringify({role:"user",content:process.argv[1]}))' "${PROMPT}")" || true
  )"
  if [[ "${post_code}" != "200" ]]; then
    printf '{"idx":%s,"task_id":"%s","ok":false,"phase":"post_message","http_code":"%s"}\n' "${idx}" "${task_id}" "${post_code}" > "${TMP_DIR}/${idx}.json"
    return 0
  fi
