...
info() { echo "[gov-source-policy-group-smoke] $*"; }
err() { echo "[gov-source-policy-group-smoke] ERROR: $*" >&2; }
...
main() {
  ...
  local objects_url policy_url groups_url
  ...
  group_name="src-policy-group-smoke-$(date +%s)"
  group_code="$(
  ...
  info "patching allow policy for created group and verifying allow"
  patch_code="$(
    curl -sS -o "${patch_resp_file}" -w '%{http_code}' \
      -X PATCH "${policy_url}" \
      -H "Authorization: Bearer ${token}" \
      -H "Content-Type: application/json" \
      --data "{\"access_mode\":\"allow_list\",\"allowed_subjects\":[{\"subject_type\":\"group\",\"subject_id\":\"${created_group_id}\"}]}" || true
  )"
  if [[ "${patch_code}" != "200" && "${patch_code}" != "204" ]]; then
  ...
  info "restoring original source_library policy and cleaning up group"
