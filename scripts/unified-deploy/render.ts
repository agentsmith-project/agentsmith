...
function buildContext(profile: UnifiedDeployProfile, env: Record<string, string>): RenderContext {
  ...
  return {
    ...
    INTERNAL_AGENT_JUICEFS_META_HOST_OVERRIDE: substrateServiceFqdn('postgresql', namespace),
    INTERNAL_AGENT_JUICEFS_META_PORT_OVERRIDE: SUBSTRATE_NATIVE_PORTS.postgresql,
    JUICEFS_BUCKET_ENDPOINT_FOR_INTERNAL_MOUNT: substrateMinioInternalMountEndpoint(namespace),
