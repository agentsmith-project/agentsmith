...
export function runRunnerContractArtifactCli(options: RunnerContractArtifactCliOptions = {}): number {
  ...
  try {
    ...
    const localManifest = readJsonObject(ARTIFACT_MANIFEST_PATH);
    const packageName = readPackageName(packageJson);
    const packageVersion = readPackageVersion(packageJson);
    const entrypoints = readEntrypoints(localManifest);

    const packOutput = runNpm(['pack', '--json', PACKAGE_DIR, '--pack-destination', config.outputDir], REPO_ROOT);
    ...
    const artifactFilename = packResult.filename;
...
function readEntrypoints(
  localManifest: JsonObject,
): CurrentRunnerContractArtifactDescriptor['entrypoints'] {
  const entrypoints = localManifest.entrypoints;
