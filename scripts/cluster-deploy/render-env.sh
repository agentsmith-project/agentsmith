...
if parsed.scheme in {"http", "https", "ws", "wss"} and parsed.netloc:
    print(urlunparse((parsed.scheme, parsed.netloc, "", "", "", "")).rstrip("/"))
else:
    print(value)
PY
}
...
K8S_EXTERNAL_API_ORIGIN_URL="$(normalize_origin_base "${K8S_EXTERNAL_API_BASE_URL}")"
K8S_EXTERNAL_API_WS_BASE_URL="$(derive_ws_base_from_http_base "${K8S_EXTERNAL_API_ORIGIN_URL}")"
set +a
...
KEYCLOAK_REALM=${KEYCLOAK_REALM}
KEYCLOAK_CLIENT_ID=${KEYCLOAK_CLIENT_ID}
AGENT_EXECUTION_HTTP_BASE_URL=${K8S_EXTERNAL_API_ORIGIN_URL}
