...
TASK_ID="${TASK_ID:-$(cat /tmp/agentsmith_last_task_id.txt 2>/dev/null || true)}"
...
if [[ -z "${PROJECT_ID}" || -z "${TASK_ID}" ]]; then
  echo "[traces-bench] missing PROJECT_ID/TASK_ID (or /tmp/agentsmith_project_id.txt / /tmp/agentsmith_last_task_id.txt)" >&2
  exit 1
fi
...
if [[ -z "${MESSAGE_ID}" ]]; then
  echo "[traces-bench] unable to resolve MESSAGE_ID (set MESSAGE_ID explicitly or ensure task has an agent reply)" >&2
  exit 1
fi

query_url="${BASE}/tasks/${TASK_ID}/traces?message_id=${MESSAGE_ID}&page_size=${PAGE_SIZE}"
if [[ -n "${RUN_ID}" ]]; then
...
echo "[traces-bench] task_id=${TASK_ID} message_id=${MESSAGE_ID} requests=${REQUESTS} concurrency=${CONCURRENCY} page_size=${PAGE_SIZE}"
...
summary_json="$(node - <<'NODE' "${TMP_DIR}" "${REQUESTS}" "${CONCURRENCY}" "${TASK_ID}" "${MESSAGE_ID}"
