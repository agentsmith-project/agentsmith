...
The files to pass as repeated `--post-deploy-product-smoke-report` inputs to `bash scripts/operator-release.sh --ga-report` are:
...
For final GA handoff, keep separate online and airgap evidence roots and pass both reports to `bash scripts/operator-release.sh --ga-report`.

Focused evidence is valid for its named scope only. It is not part of the current AgentSmith product gate; deployment/package/operator evidence is consumed only by the release-kit final GA gate, with no AgentSmith release campaign consumption implied.
