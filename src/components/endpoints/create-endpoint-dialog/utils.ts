...
export function buildCreateEndpointPayload(params: {
  ...
  return {
    ...
    model: selectedModel,
    type: params.provider === 'openai' ? 'openai' : 'custom',
    base_url: params.baseUrl.trim(),
    credential_ref: params.credentialRef,
    provider_family: params.selectedProvider.family,
    protocol: params.selectedProvider.protocol,
    meta: {
      compatibility_interface: params.selectedProvider.compatibility_interface,
