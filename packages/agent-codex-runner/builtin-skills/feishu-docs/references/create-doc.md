...
```bash
python3 /etc/codex/skills/feishu-docs/scripts/feishu_mcp.py call-tool create-doc --params '{"title":"项目计划","wiki_node":"wikcnXXXX","markdown":"## 目标\n\n- 目标 1"}'
```

Poll async task:

```bash
python3 /etc/codex/skills/feishu-docs/scripts/feishu_mcp.py call-tool create-doc --params '{"task_id":"task_xxx"}'
```
