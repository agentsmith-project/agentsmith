...
`npm run test:backend-real:core` is a focused diagnostics / evidence-owner producer rerun, not the daily real-backend gate entry and not product-side readiness / handoff sign-off. Product-side readiness / handoff sign-off remains `npm run product:ready`.
