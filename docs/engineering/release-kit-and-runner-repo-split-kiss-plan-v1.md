...
release-kit consumption、airgap load/import/offline install/deploy smoke 和
runtime/backend-real 专项。
...
- `existing_kubernetes + external_declared + airgap` 在断网环境基于 app-current inventory 完成 `verify/load/render/apply/smoke`；product-full package 当前剩余 blocker 是 release-kit consumption、airgap load/import/offline install/deploy smoke 和 runtime/backend-real 专项。
...
29. 有没有把 P5.3a release manifest skeleton/checker/start-guard 写成 runtime migration、image build/publish、AgentSmith adoption、lock update、可下载 artifact 内容 hash / 远端 artifact digest 证明或 release readiness，或者保留 `agent-task-runner` 兼容别名、发明 `descriptor_uri` / `descriptor_sha256` 字段？
...
`artifact_provenance.artifact_sha256 == subject_sha256` 只是 runner manifest subject binding /
skeleton-compatible field，不是可下载 artifact 内容 hash 或远端 artifact digest 证明；
