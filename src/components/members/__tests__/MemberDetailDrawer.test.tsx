...
describe('MemberDetailDrawer', () => {
  ...
  it('preselects matching permission template from existing member permissions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(selects[0]).toHaveAttribute('data-value', 'admin');
    });
  });
  ...
  it('keeps template unselected for custom permission set', async () => {
    render(
      <MemberDetailDrawer
        ...
        permissions={{ platform_permissions: ['project:read', 'project:member:read'] }}
      />
    );

    await waitFor(() => {
      const selects = screen.getAllByTestId('mock-select');
      expect(selects[0]).toHaveAttribute('data-value', '__none__');
    });
  });
});
