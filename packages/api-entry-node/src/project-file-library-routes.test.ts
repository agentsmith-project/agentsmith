...
describe('project-file-library-routes', () => {
  ...
  it('lists active runtime writer files through live storage without releasing runtime access', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => ({
        path: 'workspace/',
        items: [
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_live_writer',
      title: 'Entries live writer task',
    });
    ...
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries list must not release runtime access');
    });
    ...
    await handleProjectFileLibraryRoutes({
      ...
      libraryId,
      req: {
        url: '/file-libraries/entries?path=workspace',
        headers: { 'x-request-id': 'req_entries_live_writer' },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
    ...
    expect(storageAdapter.listEntries).toHaveBeenCalledWith(expect.objectContaining({
      ...
      libraryId,
      path: 'workspace/',
      requestId: 'req_entries_live_writer',
    }));
  ...
  it('returns typed entries pending for active runtime writers without release convergence', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => {
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_live_pending',
      title: 'Entries live pending task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries pending must not release runtime access');
    });
    ...
    await handleProjectFileLibraryRoutes({
      ...
      req: {
        url: '/file-libraries/entries?path=workspace%2F.artifacts',
        headers: { 'x-request-id': 'req_entries_live_pending' },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
    ...
    expect(storageAdapter.listEntries).toHaveBeenCalledWith(expect.objectContaining({
      path: 'workspace/.artifacts/',
      requestId: 'req_entries_live_pending',
    }));
  ...
  it('lists live entries through storage without converging idle runtime access', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => ({
        path: 'workspace/.artifacts/',
        items: [
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_live_no_release',
      title: 'Entries live no release task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries list must not release runtime access');
    ...
    await handleProjectFileLibraryRoutes({
      ...
      req: {
        url: '/file-libraries/entries?path=workspace%2F.artifacts',
        headers: { 'x-request-id': 'req_entries_live_no_release' },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
    ...
    expect(storageAdapter.listEntries).toHaveBeenCalledWith(expect.objectContaining({
      path: 'workspace/.artifacts/',
      requestId: 'req_entries_live_no_release',
    }));
    expect(entriesJson).toHaveBeenCalledWith(expect.anything(), 200, {
      path: 'workspace/.artifacts/',
      items: [
  ...
  it('keeps entries list behavior independent from runtime release diagnostics', async () => {
    ...
    try {
      ...
      await seedBoundTask({
        deps,
        libraryId,
        taskId: 'task_entries_no_release_warn_throw',
        title: 'Entries no release warn throw task',
      });
      deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
      deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
        throw new Error('entries list must not release runtime access');
      ...
      await handleProjectFileLibraryRoutes({
        ...
        req: {
          url: '/file-libraries/entries?path=workspace%2F.artifacts',
          headers: { 'x-request-id': 'req_entries_no_release_warn_throw' },
        } as never,
        res: createMockResponse(),
        deps,
        user: OWNER_USER,
  ...
  it('lists live entries while runtime release would remain pending', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => ({
        path: 'workspace/.artifacts/',
        items: [],
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_release_pending_ignored',
      title: 'Entries release pending ignored task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries list must not release runtime access');
    ...
    await handleProjectFileLibraryRoutes({
      ...
      req: {
        url: '/file-libraries/entries?path=workspace%2F.artifacts',
        headers: { 'x-request-id': 'req_entries_release_pending_ignored' },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
  ...
  it('keeps entries pending without retryable runtime release convergence', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => {
        throw new Error('file_library_list_pending');
      }),
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_no_retryable_release',
      title: 'Entries no retryable release task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
    ...
    try {
      ...
      await handleProjectFileLibraryRoutes({
        ...
        req: {
          url: '/file-libraries/entries?path=workspace%2F.artifacts',
          headers: { 'x-request-id': 'req_entries_no_retryable_release' },
        } as never,
        res: createMockResponse(),
        deps,
        user: OWNER_USER,
      ...
      expect(storageAdapter.listEntries).toHaveBeenCalledWith(expect.objectContaining({
        path: 'workspace/.artifacts/',
        requestId: 'req_entries_no_retryable_release',
      }));
      expect(deps.internalAgentPodManager.releasePod).not.toHaveBeenCalled();
      expect(deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding).not.toHaveBeenCalled();
      expect(storageAdapter.invalidateListReadExport).not.toHaveBeenCalled();
  ...
  it('lets consecutive live entries polls observe pending then fresh items without release invalidation', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn()
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_pending_then_fresh',
      title: 'Entries pending then fresh task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries list must not release runtime access');
  ...
  it('keeps typed pending across consecutive live entries polls without release invalidation', async () => {
    const storageAdapter = createStorageAdapter({
      listEntries: vi.fn(async () => {
        throw new Error('file_library_list_pending');
      }),
    ...
    await seedBoundTask({
      deps,
      libraryId,
      taskId: 'task_entries_live_pending_repeat',
      title: 'Entries live pending repeat task',
    });
    deps.internalAgentWorkspaceBindingManager.findWorkspaceBinding = vi.fn(async () => activeRuntimeBinding(libraryId));
    deps.internalAgentWorkspaceBindingManager.deleteWorkspaceBinding = vi.fn(async () => {
      throw new Error('entries list must not release runtime access');
  ...
  it('keeps an empty save point collection pending while an admitted save-point create is active', async () => {
    const storageAdapter = createStorageAdapter({
      createSavePoint: vi.fn(async () => ({
        operationId: 'op_save_point_active_empty',
        operationStatus: 'pending',
        savePointId: null,
      })),
      listSavePoints: vi.fn(async () => []),
      getOperationProjection: vi.fn(async () => ({
        operation_id: 'op_save_point_active_empty',
        operation_state: 'running',
        operation_type: 'save_point_create',
        resource: { type: 'repo' },
        error: null,
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      req: {
        headers: {
          'x-request-id': 'req_save_point_active_empty_create',
          'idempotency-key': 'save-point-active-empty-key',
        },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: vi.fn(),
      readBody: vi.fn().mockResolvedValue({ message: 'Before empty projection' }),
    })).resolves.toBe(true);
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      projectId: 'proj_1',
      libraryId,
      req: { headers: { 'x-request-id': 'req_save_point_active_empty_list' } } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: listJson,
    ...
    expect(storageAdapter.getOperationProjection).toHaveBeenCalledWith(expect.objectContaining({
      ...
      projectId: 'proj_1',
      operationId: 'op_save_point_active_empty',
      requestId: 'req_save_point_active_empty_list',
    }));
    expect(listJson).toHaveBeenCalledWith(expect.anything(), 409, {
      error_code: 'FILE_LIBRARY_SAVE_POINT_OPERATION_PENDING',
      message: 'file_library_save_point_create_pending',
      operation_status: 'pending',
      retry_after_ms: 2000,
    });
    expect(JSON.stringify(listJson.mock.calls[0]?.[2])).not.toMatch(/op_save_point_active_empty|repo_/);
  });

  it('keeps save point list pending over cached items while an admitted save-point create is active', async () => {
    const createSavePoint = vi.fn(async (input: { idempotencyKey: string }) => {
      if (input.idempotencyKey.includes('for-restore')) {
        return {
          operationId: 'op_save_point_cached_user',
          operationStatus: 'succeeded' as const,
          savePointId: 'sp_user_cached',
          createdAt: '2026-05-09T00:00:00.000Z',
        };
      }
      return {
        operationId: 'op_save_point_active_list_pending',
    ...
    const storageAdapter = createStorageAdapter({
      createSavePoint,
      listSavePoints: vi.fn(async () => {
        throw new Error('file_library_save_point_list_pending repo_hidden_elsewhere');
      }),
      getOperationProjection: vi.fn(async () => ({
        operation_id: 'op_save_point_active_list_pending',
        operation_state: 'running',
        operation_type: 'save_point_create',
        resource: { type: 'repo' },
        error: null,
    ...
    const created = await createReadyLibrary(deps);
    const libraryId = String(created.id);
    await createSavePointForRestore({ deps, libraryId, message: 'Cached before create' });
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      req: {
        headers: {
          'x-request-id': 'req_save_point_active_list_pending_create',
          'idempotency-key': 'save-point-active-list-pending-key',
        },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: vi.fn(),
      readBody: vi.fn().mockResolvedValue({ message: 'Before list pending projection' }),
    })).resolves.toBe(true);
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      projectId: 'proj_1',
      libraryId,
      req: { headers: { 'x-request-id': 'req_save_point_active_list_pending_list' } } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: listJson,
  ...
  it('keeps terminal save-point create failure visible while the save point collection remains empty', async () => {
    const storageAdapter = createStorageAdapter({
      ...
      getOperationProjection: vi.fn(async () => ({
        ...
        operation_type: 'save_point_create',
        resource: { type: 'repo' },
        error: { code: 'afscp_save_point_failed', message: 'raw storage failed' },
        created_at: '2026-05-09T00:01:00.000Z',
        updated_at: '2026-05-09T00:01:02.000Z',
      })),
    });
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      req: {
        headers: {
          'x-request-id': 'req_save_point_terminal_failed_create',
          'idempotency-key': 'save-point-terminal-failed-key',
        },
      } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: createJson,
      readBody: vi.fn().mockResolvedValue({ message: 'Before failed projection' }),
    })).resolves.toBe(true);
    const operationId = (createJson.mock.calls[0]?.[2] as { id?: string }).id;
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      projectId: 'proj_1',
      libraryId,
      req: { headers: { 'x-request-id': 'req_save_point_terminal_failed_list' } } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: listJson,
    ...
    await expect(handleProjectFileLibraryRoutes({
      ...
      projectId: 'proj_1',
      operationId,
      req: { headers: { 'x-request-id': 'req_save_point_terminal_failed_lookup' } } as never,
      res: createMockResponse(),
      deps,
      user: OWNER_USER,
      json: lookupJson,
    ...
    expect(JSON.stringify(lookupJson.mock.calls[0]?.[2])).not.toMatch(/op_save_point_terminal_failed|raw storage failed|repo_/);
  });
