...
Status: `p1_1_artifact_producer_passed_p2_online_target_registry_apply_evidence_spine_done_p2_registry_presence_focused_done_p2_operator_preloaded_registry_prereq_binding_done_operator_signoff_intake_guard_done_p3_airgap_bundle_render_check_done_p3_app_current_inventory_closure_done_p3_airgap_image_archive_materiality_focused_done_p5_1_start_guard_done_p5_2_formal_artifact_handoff_done_p5_3a_runner_release_manifest_skeleton_done_agentsmith_p5_3a_machine_contract_alignment_done_p5_3b_runner_runtime_fast_first_half_done`
Date: 2026-05-26
...
runner repo 完成：commit `6493603 feat: add runner runtime fast gate` 已推送到
`agentsmith-project/agentsmith-runner` main，remote CI run `26461506256`
success，jobs `Quick governance` 和 `Runner skeleton start guard` success。
该切片让 runner repo 拥有 repo-local runtime source、builtin skills、root
package/tsconfig/vitest、source-boundary guard、runtime fast focused diagnostic
和 clean-dependency start-guard guard；它不是 image build/publish、Dockerfile
migration、AgentSmith adoption lock、release contract digest adoption、release
readiness，也不是 Context Store/Files/ticket/permission semantics migration。
...
之前两个 block（legacy image id、contract_artifact 不对齐 P5.2 handoff /
...
P5.3b first half 已在 runner repo 完成：commit
`6493603 feat: add runner runtime fast gate` 已推送到
`agentsmith-project/agentsmith-runner` main；remote CI run `26461506256`
...
package/tsconfig/vitest、source-boundary guard、runtime fast focused diagnostic
和 clean-dependency start-guard guard。本地 runtime fast evidence：
`bash scripts/test-runner-runtime-fast.sh` passed，Vitest 16 files / 156 tests
passed，builtin skill Python tests 8+2+5 passed；`npm run build` passed；
...
readiness。runner repo commit `6493603 feat: add runner runtime fast gate` 已推送到
`agentsmith-project/agentsmith-runner` main；remote CI run `26461506256` 成功，
...
source-boundary guard、runtime fast focused diagnostic 和 clean-dependency
start-guard guard，但不包含 image build/publish、Dockerfile migration、
AgentSmith adoption lock、release contract digest adoption、release readiness
...
14. runner P5.3b first half runtime fast focused diagnostic 已完成：runner repo commit `6493603 feat: add runner runtime fast gate` 已推送到 `agentsmith-project/agentsmith-runner` main；remote CI run `26461506256` 成功，jobs `Quick governance` 和 `Runner skeleton start guard` 成功。完成范围是 runner repo 拥有 repo-local runtime source、builtin skills、root package/tsconfig/vitest、source-boundary guard、runtime fast focused diagnostic 和 clean-dependency start-guard guard。本地 evidence：`bash scripts/test-runner-runtime-fast.sh` passed，Vitest 16 files / 156 tests passed，builtin skill Python tests 8+2+5 passed；`npm run build` passed；clean no-node_modules `--start-guard` passed；clean no-node_modules runtime fast 按预期 fail fast，rc=2，并输出明确 dependency/artifact message。`--start-guard` 在 clean CI 不跑 runtime fast；runtime fast 需要显式 contract artifact package 和 dev deps。`@mbos/agent-runner-contract` 当前未发布到 npm，不能把普通 `npm install` 写成证据。该切片不是 image build/publish、Dockerfile migration、AgentSmith adoption lock、release contract digest adoption、release readiness，也不是 Context Store/Files/ticket/permission semantics migration。项目 pre-GA 不为 legacy/旧路径/旧命名保留长期心智负担；旧 runner paths/package names 只作为负向测试或短期迁移线索，不作为 happy path。
...
first half 已完成（runner repo commit
`6493603 feat: add runner runtime fast gate`，已推送到
`agentsmith-project/agentsmith-runner` main；remote CI run `26461506256`
success，jobs `Quick governance` 和 `Runner skeleton start guard` success）：
runner repo 已拥有 repo-local runtime source、builtin skills、root
package/tsconfig/vitest、source-boundary guard、runtime fast focused diagnostic
和 clean-dependency start-guard guard。本地 evidence：`bash scripts/test-runner-runtime-fast.sh`
passed，Vitest 16 files / 156 tests passed，builtin skill Python tests 8+2+5
...
5. P5.3a 设计收口：`image.id` 使用 `agentsmith-runner`，不保留 `agent-task-runner` 兼容别名；`contract_artifact` 绑定 P5.2 正式事实字段 `package_uri`、`package_sha256`、`package_integrity`、`descriptor_subject_sha256`，不发明 `descriptor_uri` / `descriptor_sha256`；workflow/job/generator 只要求非空，不硬编码未来 release producer；P5.3a skeleton 阶段 `artifact_provenance.artifact_sha256 == subject_sha256`，不是远端 artifact 下载证明；CLI/docs 使用 `<manifest-path>`。team review 结论是之前两个 block（legacy image id、contract_artifact 不对齐 P5.2 handoff / artifact_sha256 未绑定）已修正；最终复核无语义阻断，只提醒新增脚本必须纳入 commit，已纳入。
6. P5.1/P5.2/P5.3a 通过只解锁 runtime 迁移、image build CI 和 adoption lock 后续专项；P5.3b first half 只证明 repo-local runtime source、builtin skills、runtime fast focused diagnostic 和 clean-dependency start-guard guard，不解锁 release/adoption。Dockerfile、image build/publish、AgentSmith lock update、release contract digest adoption 和 semantics migration 仍必须在后续 P5 runtime/conformance/adoption gates 里验证。
7. P5.3b first half 已迁入 repo-local runtime source、builtin skills、root package/tsconfig/vitest 和 runner 单测 fast gate；runner Dockerfile、image build CI、AgentSmith adoption lock、release contract digest adoption、Context Store/Files/ticket/permission semantics migration 仍是后续专项。旧 `packages/agent-task-runner` 路径/包名只作为负向测试或短期迁移线索，不作为 happy path。
8. Runner repo 不允许定义 Context Store scopes、Files/file-library 行为、managed credential resolution、execution ticket 颁发或权限语义；这些语义仍由 AgentSmith contract/support API 和 fixtures 定义，runner 只消费请求级只读投影并执行本地 runtime；`mbos-context` 只能被执行/打包，不能定义 scope、write policy 或 managed credential 语义。
9. P5.1 repo-local start guard 已有 consumer / start-guard / full-gate-fail-closed 本地证据；P5.2 AgentSmith workflow handoff 已有同 run artifact 下载和 runner repo `--contract-consumer` 证据；P5.3a 已有 manifest skeleton/checker/start-guard 证据；P5.3b first half 已有 runtime fast、builtin skill Python tests、build、clean no-node_modules start-guard 和 clean dependency fail-fast 证据。后续再扩展 HOME/TASK_HOME、request-scoped env projection、credential non-persistence、Docker build 和启动缺 env fail-fast smoke。P5 后 source-boundary guard 只允许正式路径 import `@mbos/agent-runner-contract`，其他 `@mbos/*` import 失败。
...
15. 迁移期保留本地 dev 启动说明，但正式 release contract 只能接受 runner manifest + lock adoption；本地 dev 路径不能作为 release proof。P5 runner repo dev command 可用后删除 AgentSmith 本地开发启动入口，或归位为 docs-only 本地启动说明。
...
2. release kit adapter 完成 parity、release-kit repo-local verdict 已拥有 deployment/package/operator、回滚路径明确前，不删除底层部署合同语义；明确后从 AgentSmith active status/workflow 移除或隐藏 transition-only focused diagnostics / 过渡期专项诊断，只保留必要 fail-fast 负向测试。项目仍是 pre-GA，不为已移除旧输入、旧路径或旧文档/旧脚本引用保留长期心智负担；P6 后旧 profile、旧 writer、旧 command、旧 doc/script 引用只能作为 docs-only 迁移线索或负向测试，任何短期暂留都必须有删除责任、退出条件、删除时机和验收证据；若保留合同，必须归一到 `target_cluster` / `substrate_source` / `distribution` 三轴新模型。
...
6. P4/P5.0 已证明 runner contract artifact 有 URI/digest/integrity/provenance
   ...
   runtime source、builtin skills、root package/tsconfig/vitest、source-boundary guard、
   runtime fast focused diagnostic 和 clean-dependency start-guard guard；`--start-guard`
...
已完成 -> Dockerfile migration / image build CI / adoption lock 后续专项 -> P6 收口。P4 runner contract artifact
...
fast/source-boundary 已完成。P2 online spine、P3 render-check、P3 app-current inventory
...
CLI/docs 使用 `<manifest-path>`；team review 的两个 block（legacy image id、
...
P5.3b first half runtime fast focused diagnostic 已完成：runner repo commit
`6493603 feat: add runner runtime fast gate` 已推送到
`agentsmith-project/agentsmith-runner` main；remote CI run `26461506256` 成功，
...
source-boundary guard、runtime fast focused diagnostic 和 clean-dependency
start-guard guard。本地 evidence：`bash scripts/test-runner-runtime-fast.sh`
passed，Vitest 16 files / 156 tests passed，builtin skill Python tests 8+2+5
passed；`npm run build` passed；clean no-node_modules `--start-guard` passed；
clean no-node_modules runtime fast expected fail-fast rc=2 with explicit
dependency/artifact message。`--start-guard` 在 clean CI 不跑 runtime fast；
...
旧 runner paths/package names 只作为负向测试或短期迁移线索，不作为 happy path。
