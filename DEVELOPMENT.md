...
As of the current internal release candidate, `Audit` and `Usage` are backed by real local `api-entry-node` routes (product-usable v1), while `Members` and `Resource Policy` are partially backed with baseline reads/writes. They should be treated as `partial` in real local-backend mode, with scope limits documented in the release capability matrix. `Resource Policy` partial now includes endpoint/agent allow-list enforcement (user/group subjects) and endpoint `requests_per_minute` rate limiting. Backend project route authorization also now has a minimal real closure path: group membership + permission templates can expand granted permissions (allow-only union model) via the shared project authz resolver.
