...
const mockAgentRunnerList = vi.fn().mockResolvedValue({ items: [], total: 0, page: 1, page_size: 500, has_more: false });
...
vi.mock('@/lib/api/endpoints/agent-runners', () => ({
  AgentRunnerAPI: vi.fn().mockImplementation(function () { return { list: mockAgentRunnerList }; }),
}));
...
describe('ChatPage', () => {
  beforeEach(() => {
    ...
    mockAgentRunnerList.mockClear();
    mockAgentRunnerList.mockResolvedValue({ items: [], total: 0, page: 1, page_size: 500, has_more: false });
  });
  ...
  it('renders compact layout with new-thread action', async () => {
    ...
    expect(screen.getByTestId('chat__surface').className).not.toContain("[&_[data-testid='chat__main-pane']]:!bg-none");
    expect(screen.getByTestId('chat__surface').className).not.toContain("[&_[data-testid='chat__composer-recovery']]:!bg-transparent");
  ...
  it('keeps chat LLM-only and does not query Agent Runners', async () => {
    render(<ChatPage params={Promise.resolve({ workspace: 'ws_1', project: 'proj_1', locale: 'en' })} />, { wrapper: createWrapper() });

    await waitFor(() => {
      expect(screen.getByTestId('chat__main-pane')).toBeInTheDocument();
    });
    expect(mockAgentRunnerList).not.toHaveBeenCalled();
  });
});
