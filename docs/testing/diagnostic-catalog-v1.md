...
| Maintainer diagnostic `npm run test:unified-deploy:product-flows` | You need the canonical deployed product smoke matrix without a heavy release campaign. | This proves the seven canonical post-deploy product flows: login/profile, workspace/project, Chat via llmup, managed runner task, Files, audit, and usage. |
