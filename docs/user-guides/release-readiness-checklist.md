...
- release-kit final GA 的 `use_existing` evidence 可以来自本地 actual deployment with operator-provided existing substrates；通过当前 release inputs、substrate truth digest、deployment path report 和 smoke 即可，旧 evidence 不可复用。外部云托管服务只是常见实施配置，不是当前 GA 必过 gate；`use_existing` 不运行 substrate installer，也不消费 `install_substrates` namespace-scoped installer evidence。
