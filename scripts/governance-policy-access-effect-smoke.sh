...
main() {
  ...
  patch_code="$(
    curl -sS -o "${patch_resp_file}" -w '%{http_code}' \
      ...
      -H "Authorization: Bearer ${token}" \
      -H "Content-Type: application/json" \
      --data "{\"access_mode\":\"allow_list\",\"allowed_subjects\":[{\"subject_type\":\"user\",\"subject_id\":\"${user_id}\"}],\"rate_limits\":{\"rules\":[{\"key\":\"endpoint.requests_per_minute\",\"value\":1000}]},\"quota_limits\":{\"rules\":[]}}" || true
  )"
  if [[ "${patch_code}" != "200" && "${patch_code}" != "204" ]]; then
