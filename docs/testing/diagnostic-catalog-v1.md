...
Diagnostic commands are not final conclusions. They help you find the failing layer, prove a focused fix, and decide which clean verification entrypoint must be rerun next. Rows that mention internal adapters, unified deploy producers, or `npm run test:*` owner commands are maintainer diagnostics; do not treat this catalog as the default command directory for ordinary development or product readiness / handoff sign-off.
...
| Maintainer diagnostic `npm run test:unified-deploy:product-flows` | You need the canonical deployed product smoke matrix without a heavy release campaign. | This proves the seven canonical post-deploy product smokes: login/profile, workspace/project, `provider_neutral_endpoint` / provider-neutral Endpoint, managed runner task, Files, audit, and usage. Internal diagnostics may still cite the source flow `chat_via_llmup`. |
