...
local_redis_require_simple_password REDIS_PASSWORD "${REDIS_PASSWORD}" "[internal-real-gate]"
...
ensure_internal_default_workspace_for_afscp() {
  ...
  (
    cd "${ROOT_DIR}" && \
      ...
      REDIS_URL="redis://:${REDIS_PASSWORD}@localhost:${INTEGRATION_REDIS_PORT}" \
...
run_internal_spec() {
  ...
  (
    cd "${ROOT_DIR}" && \
      ...
      MONGO_URL="${MONGO_URL}" \
      MONGO_DB_NAME="${MONGO_DB_NAME}" \
      REDIS_URL="redis://:${REDIS_PASSWORD}@127.0.0.1:${INTEGRATION_REDIS_PORT}" \
      MINIO_ENDPOINT="127.0.0.1" \
      MINIO_PORT="${INTEGRATION_MINIO_API_PORT}" \
      KEYCLOAK_BASE_URL="${KEYCLOAK_BASE_URL}" \
      KEYCLOAK_URL="${KEYCLOAK_BASE_URL%/}/realms" \
