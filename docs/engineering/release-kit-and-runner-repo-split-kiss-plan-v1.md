...
install/deploy smoke、AgentSmith adoption lock、release contract runner digest adoption、AgentSmith 侧 support API /
...
adoption lock、release contract runner digest adoption 和 AgentSmith 侧 support API /
projection contract 一致性收口后续专项，按 KISS 小切片推进。
...
8. P5.3a/P5.3b first half/boundary closure/P5 image smoke/P5 publish manifest
   ...
   先进入 AgentSmith manifest/lock adoption gate，产出锁定 runner digest 后，
   才进入 release contract runner digest adoption 和 release-kit 消费串联。
