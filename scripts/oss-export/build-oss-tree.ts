  ...
  if (existsSync(clusterBuildImagesPath)) {
    writeFileSync(
      ...
      [
        ...
        'docker_build_local --build-arg NODE_BASE_IMAGE="${RUNNER_NODE_BASE_IMAGE}" -t "${RUNNER_BASE_IMAGE}" -f "${APP_SOURCE_DIR}/infra/runner/Dockerfile.notebook-codex-runner-base" "${APP_SOURCE_DIR}"',
...
function rewriteDocs(): void {
  ...
  writeFileSync(join(OSS_TARGET_DIR, 'docs', 'RUNNER_RUNTIME.md'), `# Runner Runtime Paths\n\n## Modes\n\n| Mode | Workspace root (cwd) | CODEX_HOME | HOME | Artifacts | Credentials | Builtin skills visible to Codex by default |\n| --- | --- | --- | --- | --- | --- | --- |\n| External bare | \`${'${MBOS_AGENT_WORKSPACE_ROOT:-$HOST_HOME/ags-workspaces}/<workspace_dir_name>/'}\` | \`<cwd>/.codex/\` | \`<cwd>/\` | \`<cwd>/.artifacts/\` | \`<cwd>/.mbos/credentials/\` | \`$HOME/.agents/skills\` |\n| External docker | \`${'${MBOS_AGENT_WORKSPACE_ROOT:-/workspace/ags-workspaces}/<workspace_dir_name>/'}\` | \`<cwd>/.codex/\` | \`<cwd>/\` | \`<cwd>/.artifacts/\` | \`<cwd>/.mbos/credentials/\` | \`$HOME/.agents/skills\` |\n| Internal | \`/workspace/\` | \`/workspace/.codex/\` | \`/workspace/\` | \`/workspace/.artifacts/\` | \`/workspace/.mbos/credentials/\` | \`$HOME/.agents/skills\` |\n\n## How Codex uses these paths\n\n| Path | Purpose | How Codex consumes it |\n| --- | --- | --- |\n| \`cwd\` | Active project files and persistent home | Command execution, file reads, edits, user installs, and repo discovery all happen relative to this root. |\n| \`CODEX_HOME\` | Task-scoped Codex state | Session state, sqlite state, snapshots, and generated task config live here. Notebook resume stays within the current task scope because \`resume --last\` resolves inside this directory. |\n| \`HOME\` | Task-scoped runner home | The runner pins \`HOME == cwd\`. User-mode installs, discovery roots, and mutable task state all stay inside the workspace. |\n| Artifacts directory | Final outputs | The runner writes notebook outputs here so the application can surface them in the task UI. |\n| Credentials directory | Task credential material | AgentSmith syncs third-party credentials here and helper scripts consume them without leaving the current home. |\n| \`$HOME/.agents/skills\` | Builtin skills visible to Codex | The runner mirrors builtin skills into the current home so pod or container restarts can recover from the persistent workspace. |\n\n## User-mode install note\n\nWhen an agent needs Python, Node, or Rust tooling, install it in user mode under the current home. The runner exports user-scoped defaults such as \`PYTHONUSERBASE=$HOME/.local\`, \`npm_config_prefix=$HOME/.local\`, \`CARGO_HOME=$HOME/.cargo\`, and \`RUSTUP_HOME=$HOME/.rustup\`.\n`);
