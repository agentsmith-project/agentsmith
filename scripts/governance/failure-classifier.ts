...
const FAILURE_PATTERNS: FailurePattern[] = [
  ...
  // Must come before token/network to avoid false positives
  {
    ...
    recommendation: 'Backend error: Check API logs at artifacts/backend-real/current/demo/api.log',
    documentationLink: '/docs/troubleshooting-guide-v1.md#backend-issues',
    priority: 100,
    steps: [
      '1. Check API logs: `tail -100 artifacts/backend-real/current/demo/api.log`',
      '2. Check backend status: `make local-manual-status`',
  ...
  // TOKEN - Authentication issues
  {
    ...
    recommendation: 'Token issue: Run `make agent-runner-refresh-token` and retry.',
    ...
    steps: [
      ...
      '2. Run: `make agent-runner-refresh-token`',
      '3. If refresh fails, check Keycloak is running: `make local-manual-status`',
      '4. Verify environment variables: KEYCLOAK_URL, KEYCLOAK_REALM, KEYCLOAK_CLIENT_ID',
      '5. Check token file: `cat artifacts/backend-real/current/token.txt`',
    ],
  },
  // NETWORK - Connection issues (must come before timeout for ETIMEDOUT)
  {
    ...
    recommendation: 'Network issue: Check services are running (`make local-manual-status`).',
    documentationLink: '/docs/troubleshooting-guide-v1.md#network-issues',
    priority: 90,  // Higher than timeout
    steps: [
      '1. Check service status: `make local-manual-status`',
      ...
      '5. Restart services if needed: `make local-manual-down && make local-manual-up && make local-manual-seed-agent-task`',
