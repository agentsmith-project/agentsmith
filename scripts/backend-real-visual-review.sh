...
info "PRESET_ENDPOINT_API_KEY=<redacted> RELEASE_REAL_VISUAL_ARTIFACT_DIR='${ARTIFACT_DIR}' INTEGRATION_API_PORT='${API_PORT}' INTEGRATION_WEB_PORT='${WEB_PORT}' NEXT_DEV_MEMORY_PROFILE='${NEXT_DEV_MEMORY_PROFILE}' bash scripts/run-internal-agent-task-real-gate.sh --visual-review"
(
  cd "${ROOT_DIR}" && \
