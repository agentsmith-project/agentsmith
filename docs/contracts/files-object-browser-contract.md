...
Playwright (`e2e/files.spec.ts`) must cover:
...
- Library switch restore rule:
  - restore folder/query/selection state only when switching libraries within the same Files page session
  - refresh or leaving Files resets to default entry state (no cross-session restore)
  - only `library_id` remains URL-persisted; folder/query/sort are session-local state

Integration coverage (`e2e/integration-files.spec.ts`) must validate the same flow against
