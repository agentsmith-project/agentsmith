...
describe('Permission Change Propagation', () => {
  ...
  it('should propagate permission changes within 1 request cycle', async () => {
    const request: PermissionUpdateRequest = {
      ...
      workspace_id: 'ws-1',
      project_id: 'proj-1',
      permissions: ['project:governance:update'], // Add new permission
      mode: 'custom',
    };
    ...
    expect(currentPerms.permissions).toContain('project:governance:update');
  });
  ...
  it('should reflect removed permission in next authorization check', async () => {
    ...
    const result = await checkPermission({
      member_id: 'user-1',
      permission: 'project:governance:update', // This should now be denied
      resource_context: {
        workspace_id: 'ws-1',
        project_id: 'proj-1',
      },
  ...
  it('should propagate template role changes within 1 request cycle', async () => {
    ...
    // Admin template should include project:governance:update
    expect(currentPerms.permissions).toContain('project:governance:update');
  });
