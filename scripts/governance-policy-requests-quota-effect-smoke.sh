...
info() { echo "[gov-policy-requests-quota-smoke] $*"; }
err() { echo "[gov-policy-requests-quota-smoke] ERROR: $*" >&2; }
...
usage_requests_for_user_endpoint() {
  local base="$1"
  local token="$2"
  local endpoint_id="$3"
  local user_id="$4"
  ...
  if [[ "${code}" != "200" ]]; then
    err "usage query failed while reading current requests (HTTP ${code})"
    cat "${out_file}" >&2 || true
    rm -f "${out_file}"
    return 1
  fi
  cat "${out_file}" | json_get "const items=Array.isArray(data.items)?data.items:[]; const row=items.find(i=>String(i.resource_type||'')==='endpoint'&&String(i.resource_id||'')==='${endpoint_id}'&&String(i.end_user_id||'')==='${user_id}'); process.stdout.write(String(Number(row?.requests||0)));"
  rm -f "${out_file}"
}
...
main() {
  ...
  current_requests="$(usage_requests_for_user_endpoint "${base}" "${token}" "${endpoint_id}" "${user_id}")"
  info "current endpoint requests for user ${user_id} today: ${current_requests}"

  if [[ "${current_requests}" == "0" ]]; then
    info "no endpoint request usage found; sending warm-up request"
    local warmup_code
    warmup_code="$(
    ...
    if [[ "${warmup_code}" != "000" && ! "${warmup_code}" =~ ^2[0-9][0-9]$ ]]; then
    ...
    sleep 2
    current_requests="$(usage_requests_for_user_endpoint "${base}" "${token}" "${endpoint_id}" "${user_id}")"
    info "endpoint requests after warm-up: ${current_requests}"
  fi
  if [[ "${current_requests}" == "0" ]]; then
    err "unable to observe endpoint request usage for user ${user_id}; cannot deterministically trigger policy quota"
    exit 1
  fi

  local quota_limit patch_code
  quota_limit="${current_requests}"
  info "patching endpoint policy with requests/day quota=${quota_limit}"
  patch_code="$(
    curl -sS -o "${patch_resp_file}" -w '%{http_code}' \
      ...
      --data "{
        ...
        \"quota_limits\":{\"rules\":[{\"key\":\"endpoint.requests_per_day\",\"value\":${quota_limit}}]}
      }" || true
  )"
  ...
  info "sending endpoint request (should hit requests/day policy quota)"
  req_code="$(
  ...
  if [[ "${req_code}" != "429" ]]; then
    err "request did not hit requests/day policy quota (HTTP ${req_code})"
    cat "${req_file}" >&2 || true
  ...
  local body_quota_key
  body_quota_key="$(cat "${req_file}" | json_get 'process.stdout.write(String(data.quota_key||""))' || true)"
  if [[ "${body_quota_key}" != "endpoint.requests_per_day" ]]; then
    err "unexpected quota_key on quota-limited response: ${body_quota_key}"
    cat "${req_file}" >&2 || true
    rm -f /tmp/gov_policy_quota_smoke_headers.$$ || true
    exit 1
  fi
  ...
  local audit_has
  audit_has="$(cat "${audit_file}" | json_get "const ok=Array.isArray(data.items)&&data.items.some(i=>i.action==='resource_policy.quota_exceeded'&&String(i.resource_id||'')==='${endpoint_id}'&&String(i.actor_id||'')==='${user_id}'&&String((i.metadata_json||{}).quota_key||'')==='endpoint.requests_per_day'); process.stdout.write(ok?'1':'0');")"
  if [[ "${audit_has}" != "1" ]]; then
