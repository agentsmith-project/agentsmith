...
Status: `p1_1_artifact_producer_passed_p2_online_target_registry_apply_evidence_spine_done_p2_registry_presence_focused_done_p2_operator_preloaded_registry_prereq_binding_done_operator_signoff_intake_guard_done_p3_airgap_bundle_render_check_done_p3_app_current_inventory_closure_done_p3_airgap_image_archive_materiality_focused_done_p5_1_start_guard_done_p5_2_formal_artifact_handoff_done_p5_3a_runner_release_manifest_skeleton_done`
Date: 2026-05-26
...
success，jobs `Quick governance` 和 `Runner start guard` success）。P2 online target-registry
...
13. DeepSeek/LLM real lane 没有 tracked changes；AgentSmith defaults 和 ignored local env 使用 DeepSeek endpoint/model，LLMUP real compatibility smoke 15 passed / 0 failed / 1 skipped，未提交 secret。
14. AgentSmith commit `7cf783c2` 已推送；此前远端 Contracts Check run `26447963233` 因 GitHub Actions checkout/auth 403 失败，不是治理脚本失败。
...
| Runner image lock | AgentSmith | 正式 lock identity 必须收敛到 `agentsmith-runner`；目标为 `agentsmith-runner-image.lock` 或等价 canonical lock path。现有 `scripts/governance/__fixtures__/release-boundary/agent-task-runner-image.lock` / `image_id=agent-task-runner` 只能作为 pre-GA 过渡 fixture 或负向迁移线索 | AgentSmith adoption PR | AgentSmith release contract generator、backend-real | lock 与 runner manifest/release contract digest 不一致、接受 `agent-task-runner` 作为正式 lock identity、保留兼容别名 |
...
11. AgentSmith 后续 adoption lock 必须把正式 lock identity 收敛到 `agentsmith-runner`（目标 `agentsmith-runner-image.lock` 或等价 canonical lock path），并用 runner release manifest 比对 image digest、contract version、contract artifact digest/provenance、Codex version 和 fail-fast adoption policy。现有 `agent-task-runner-image.lock` / `image_id=agent-task-runner` 只保留为 pre-GA 过渡 fixture 或负向迁移线索；adoption 后删除或拒绝 `agent-task-runner` 兼容别名，不做双轨支持。
