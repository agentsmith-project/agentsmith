...
const STORY_FILE = path.resolve(
  process.cwd(),
  'e2e/stories/backend-real/agent-task-terminal-truth-unavailable-retry.story.md',
);

describe('Agent Task terminal truth-unavailable retry story', () => {
  it('defines a backend-real journey for explicit retry before terminal truth unlocks the task again', () => {
    ...
    expect(story.storyId).toBe('agent-task-terminal-truth-unavailable-retry');
    ...
    expect(
      story.steps.find((step) => step.stepId === 'keep-run-and-delete-fail-closed-while-terminal-truth-is-missing')
        ?.target,
    ).toBe('agent-tasks__conversation-blocked-state');
  ...
  it('binds the truth-unavailable real-lane spec to the canonical story instead of a one-off integration branch', async () => {
    ...
    expect(storySource).toContain('"storyId": "agent-task-terminal-truth-unavailable-retry"');
    expect(storySource).toContain('"family": "agent-task-terminal-workspace"');
    ...
    expect(specSource).not.toContain('/notebook');
    expect(specSource).not.toContain('notebook__');
  });
});
