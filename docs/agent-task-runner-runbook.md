...
| --- | --- | --- | --- | --- |
| Files | Return typed `file_library_list_pending` / read-export pending from the live Files read owner; do not return stale `200 items: []`. | Keep browse/list/download on the live Files read/export path; runtime release is not a default list precondition. | Treat as no active writer for Files reads and use only the Files read/export path; do not create an executable connector. | Treat as no active writer for Files reads; do not synthesize an executable connector. |
...
| AFSCP workspace binding | Keep the owning operation pending until terminal success or typed failure/blocker. | Continue explicit release/restore convergence through the workspace binding owner until terminal `released`, `revoked`, `expired`, or `deleted`. | Treat as no active writer for Files reads; executable attachment must use the Agent Task sandbox owner path. | Treat as no active writer for Files reads; executable attachment must use the Agent Task sandbox owner path. |
| Read export | Return typed pending when live read/export/projection is not ready. | Keep live read/export semantics; explicit release fences do not make browse/list/download pretend success. | Create or reuse the live read/export path without inventing an executable attachment. | Create or reuse the live read/export path if available; otherwise return typed pending/unavailable. |
...
- AFSCP workspace binding `releasing` / `release_pending`: continue release convergence through the workspace binding owner for explicit runtime-access release and restore-related control operations until the binding is terminal (`released`, `revoked`, `expired`, or `deleted`).
- AFSCP workspace binding release retryable infra failure: explicit release/restore control paths surface typed pending or retryable runtime readiness evidence; browse/list/download do not start release as a side effect.
- AFSCP workspace binding completed release fence: if the same task owner later opens a terminal or starts a managed run, the Agent Task sandbox owner path may resume that completed fence back to executable access before the lease expires; active `releasing` fences still return a typed conflict/pending result.
- Agent Task terminal/session close release pending or sandbox rate limited: keep bounded, increasing backoff on the close/release path, wait on the in-flight DELETE/release request instead of overlapping duplicate release calls, and classify a first passing rerun as runtime flake; repeated 429/release-pending evidence is a stability blocker. If terminal close final truth has already landed, focused cleanup may continue while workspace-binding release remains under runtime pending/readiness evidence.
- AFSCP workspace binding `offline` / `not_found`: treat as no active writer for Files reads; any new executable attachment must be created by the Agent Task sandbox owner path, not by Files inventing a connector.
...
- Files live tree: current working tree means the AFSCP repo payload root / managed runner HOME workspace tree, including files written under `$HOME/workspace/...` and `$HOME/workspace/.artifacts/...`.
- Files read export `pending`: return typed pending to the caller from the live read/export owner. Do not trigger runtime-access release or return stale `200 items: []`.
- Files entries list/download with an idle or active runtime writer: read the live Files path directly; do not converge runtime-access release before browse/list/download.
- Files runtime release with AFSCP revoke idempotency conflict: map the AFSCP 409 `conflict` evidence to retryable runtime readiness for explicit release/restore control paths; browse/list/download remain live-read operations.
- Files save point/restore: snapshot/restore the live payload root when owner readiness allows it, or keep the public operation typed pending/unavailable; do not project stale empty success.
