...
| chat | stream chat via external agent (thread-level binding) | `project:endpoint:use` + `project:agent:manage` | `POST /chat/sessions/{id}/messages/stream` (when `external_agent_id` is set in session) | fail-fast stream error with explicit message |
...
| files | file CRUD in selected library | `project:manage` | `POST/PATCH/DELETE /source-libraries/{libraryId}/files*` | mutating controls disabled |
| files | start/cancel AIReady job | `project:manage` | `POST /source-libraries/{libraryId}/ai-ready-jobs`, `POST /source-libraries/{libraryId}/ai-ready-jobs/{jobId}:cancel` | mutating controls disabled; keep job status visible |
| agents | view/use agents | `project:agent:manage` | `GET /agents*`, `GET /agents/{id}/runtime-config`, `GET /agents/{id}/connection-info` | page-level permission denied |
...
| resource policy | view/update endpoint resource policy | `project:manage` | `GET/PATCH /resources/endpoint/{id}/policy` | mutating controls disabled |
| credentials | view/manage credentials | `project:manage` | `GET/POST/DELETE /credentials*` | page-level permission denied |
| members | view members | `project:manage` | `GET /members*`, `GET /groups`, `GET /permission-templates`, `GET /quota-templates*` | page-level permission denied |
| members | manage members/templates/groups | `project:manage` | `/members/*`, `/invites`, `/join-requests/*`, `/groups*`, `/permission-templates*`, `/quota-templates*` | mutating controls disabled |
| settings | view/update project settings | `project:manage` | `GET/PATCH /projects/{id}` | page-level permission denied or save disabled |
| settings | delete project from settings | `project:manage` | `DELETE /projects/{id}` | delete denied |
...
|-----|----------------|------------------------|-------------------|-------------------|
| overview | view runtime health and control plane | `project:manage` | `GET /projects/{id}`, `GET /runtime/*` | tab-level permission denied, show access message |
...
| Runtime Console (merged) | Operate (4 pages) | Operate | `project:manage`, `project:endpoint:use` |
...
1. MVP project permission set is fixed to:
   ...
   - `project:agent:manage`
   - `project:manage`
2. For settings delete, FE and backend should both enforce `project:manage`.
