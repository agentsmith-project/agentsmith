...
14. P5 runtime 后续仍需逐项迁移或收口：local-kind runner image publish/adoption、API 默认 managed runner image、internal agent pod health/imageID probe、`agent:task-runner` dev script、skills diagnostics、`buildAgentRuntimeEnv` ownership。`agent:task-runner` dev script 是短期待删说明，不是 release identity；release-kit managed runner image closure consumption 已完成但不替代 runtime/backend-real/task execution/Codex smoke。
