    ...
    jobs: [
      {
        ...
        requiresSecrets: true,
        evidenceRequired: true,
        evidenceFamilies: ['product_readiness_report'],
        artifactPaths: [
          '${{ env.RELEASE_CAMPAIGN_ROOT }}/product-readiness/product-readiness-report.json',
          '${{ env.RELEASE_CAMPAIGN_ROOT }}/summary.json',
          '${{ env.RELEASE_CAMPAIGN_ROOT }}/gate-release-full/result.json',
        ],
        blockingFor: ['manual', 'handoff'],
        scheduled: false,
        productReadinessBlocking: false,
        notes: 'Runs AgentSmith product readiness with a downloaded release contract and uploads the final GA product readiness handoff report plus referenced summary/result files.',
      },
    ],
  }),
  defineCurrentCIWorkflow({
