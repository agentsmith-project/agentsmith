---
name: mbos-context
description: Use to read or write AgentSmith Context Store entries for member/task scope, and to read project_member/project/workspace context. Trigger when the user asks to save preferences, notes, simple credentials, task memory, shared guidance, or personal project bindings without browsing local files.
---
...
6. External service credentials are request-scoped environment values or ordinary Context Store entries only when explicitly provided; fail fast when the needed entry or env var is missing.
...
- Do not invent provider-specific credential projections; use explicit Context Store keys or request-scoped env vars.
