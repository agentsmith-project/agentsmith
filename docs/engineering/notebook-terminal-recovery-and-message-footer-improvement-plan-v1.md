# Notebook Terminal Recovery and Message Footer Improvement Plan v1

更新时间：2026-05-01 PDT
状态：`handoff_plan_ready`
适用范围：Notebook terminal 恢复体验、Browser terminal WS reconnect handshake、前端 terminal focus/fit 行为、AI 消息气泡 active footer 布局稳定性
...
- notebook terminal 相关 OpenAPI / WS / SSE contract
...
当前 notebook terminal 的问题不是“连接没恢复”，而是“可继续工作的终端视图没有恢复”。刷新页面、切换 conversation / terminal 或切 tab 后，后端只向前端发送 synthetic `started`，没有 replay 或 screen snapshot；前端 xterm 重新挂载后 buffer 是空的，用户会看到运行中任务、交互式程序或 prompt 内容消失。这会让用户误判为任务丢失，尤其在长命令、交互式程序、需要继续输入的场景中风险很高。
...
> AgentSmith terminal 是 Notebook task 内的短生命周期 shell 执行入口。刷新、断线、切换后，系统承诺恢复可信状态和可用控制；不承诺完整屏幕现场、长期日志、跨任务工作台或平台托管 shell 会话。
...
- `NotebookTerminalService` 实时将 output `sendToBrowserSocket`，但没有 server-side replay buffer。
...
- 当前只把 terminal 当作 streaming endpoint，没有形成 Notebook task-scoped terminal session contract：即在某个 notebook task 范围内，由后端授权和路由的短生命周期 terminal session 契约。
...
- `docs/contracts/agent-execution-protocol.md`
- notebook terminal 相关 OpenAPI / WS contract
- 后端 token、session owner、task/project/workspace scope、`project:terminal:use` 权限检查结果
...
- `terminal.reconnect` 必须携带 `terminal_session_id`、`view=notebook.task_terminal`、当前 `rows` / `cols`，并可携带 `after_seq`。字段类型、取值范围、正式 view 常量、session 归属、scope 和权限校验失败时，服务端必须返回明确 `terminal.error`，随后关闭连接或用明确 close reason 关闭。
- `GET list/get terminal session` 会返回可 attach 的 `ws_url` / ticket，必须要求 `project:terminal:use`；`delete terminal session` 不下发 ticket，不能作为后续可交互 attach、`terminal.stdin`、`terminal.resize` 的权限证明。attach/reconnect、`terminal.stdin`、`terminal.resize` 每次都必须重新校验 token、session owner、scope 和 `project:terminal:use`。
...
- 字段校验失败（包括 `view` 不是 `notebook.task_terminal`）：返回 `invalid_reconnect_payload` error 并关闭。
...
- `GET list/get` 缺少 `project:terminal:use` 时不能下发 `ws_url` / ticket；`delete` 不下发 ticket，也不能让后续 attach/reconnect、`terminal.stdin`、`terminal.resize` 绕过 `project:terminal:use` 复校验。
...
- AgentSmith 需要治理的是 Notebook task-scoped terminal session contract，而不是替用户选择或编排 shell 内部的会话工具。
...
3. 服务端校验 payload、token、ticket、session owner、task/project/workspace scope 和 `project:terminal:use`。
...
| `view` | 正式 browser-facing view 常量；当前唯一合法值是 `notebook.task_terminal`，服务端必须严格校验，不能只判非空 |
...
- `GET list/get terminal session` 因下发 `ws_url` / ticket 必须要求 `project:terminal:use`；`delete terminal session` 不下发 ticket，不能被复用为 `terminal.stdin` / `terminal.resize` 权限证明。
- attach/reconnect、`terminal.stdin`、`terminal.resize`、interrupt/paste 必须重新校验 token、session owner、task/project/workspace scope 和 `project:terminal:use`。
...
- attach/reconnect、`terminal.stdin`、`terminal.resize` 权限 guard：token、owner、scope、`project:terminal:use`。
- footer 长文本布局 class / variant 的确定性单元测试。

### 8.2 Backend service

- `NotebookTerminalService` 在实时 send 之外同步写入 bounded ring。
...
- `GET list/get` 缺少 `project:terminal:use` 时不能下发 `ws_url` / ticket；`delete` 不下发 ticket，后续 attach/reconnect、`terminal.stdin`、`terminal.resize` 仍必须按 `project:terminal:use` 失败。
...
涉及 ticket、scope、execution context、notebook runner terminal attach/reconnect 的改动，至少补跑 notebook runner focused diagnostics：

- `npm run test:notebook:runner:fast`
- 如果触及 backend-real attach/reconnect、ticket 下发或权限撤销路径，再跑 `npm run test:notebook:runner:backend-real`
...
4. attach/reconnect、`terminal.stdin`、`terminal.resize` 重新校验 token、session owner、scope 和 `project:terminal:use`；`GET list/get` 下发 `ws_url` / ticket 时要求 `project:terminal:use`，`delete` 不下发 ticket 且不能替代可交互 attach。
...
12. contract/OpenAPI/generated checks 通过；涉及 ticket/scope/execution context 时 notebook runner focused diagnostics 通过。
...
- `GET list/get` 下发 `ws_url` / ticket 时必须要求 `project:terminal:use`；`delete` 不下发 ticket，不能被当作 attach/reconnect、`terminal.stdin`、`terminal.resize` 权限。
...
8. 不把 `delete` 这类不下发 ticket 的操作当作可交互 attach 权限；`GET list/get` 下发 `ws_url` / ticket 时必须要求 `project:terminal:use`。
...
5. 涉及 ticket/scope/execution context 时补 notebook runner diagnostics：`test:notebook:runner:fast`，必要时 `test:notebook:runner:backend-real`。
...
3. Focused validation：MessageItem unit、相关 notebook e2e、受影响 visual scenario。
