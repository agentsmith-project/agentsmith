...
main() {
  ...
  deny_code="$(
    curl -sS -o "${deny_file}" -w '%{http_code}' \
      ...
      -H "Authorization: Bearer ${token}" \
      -H "Content-Type: application/json" \
      --data "$(node -e 'console.log(JSON.stringify({model:process.argv[1],messages:[{role:"user",content:"policy access deny smoke"}]}))' "${BACKEND_REAL_MODEL}")" || true
  )"
  ...
  for ((allow_attempt=1; allow_attempt<=ALLOW_VERIFY_RETRIES; allow_attempt++)); do
    allow_code="$(
      curl -sS -o "${allow_file}" -w '%{http_code}' \
        ...
        -H "Authorization: Bearer ${token}" \
        -H "Content-Type: application/json" \
        --data "$(node -e 'console.log(JSON.stringify({model:process.argv[1],messages:[{role:"user",content:"policy access allow smoke"}]}))' "${BACKEND_REAL_MODEL}")" || true
    )"
