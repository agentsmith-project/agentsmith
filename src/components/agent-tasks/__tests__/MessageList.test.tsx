...
vi.mock('../MessageItem', () => ({
  MessageItem: ({ message, streamingContent, disabled, activeRunView }: any) => (
    <div data-testid={`message-item-${message.id}`}>
      <div data-message-actor>{message.actor}</div>
...
describe('MessageList', () => {
  ...
  describe('Message Rendering', () => {
    ...
    it('passes message actor to MessageItem', () => {
      renderComponent();

      expect(screen.getByTestId('message-item-msg-1').querySelector('[data-message-actor]')?.textContent).toBe('user');
      expect(screen.getByTestId('message-item-msg-2').querySelector('[data-message-actor]')?.textContent).toBe('runner');
    });
  ...
  describe('Streaming State', () => {
    ...
    it('shows streaming indicator for runner messages', () => {
      const _streamingAgentMsg: TaskActivityItem = {
        id: 'msg-3',
        task_id: 'task-1',
        kind: 'runner_output',
    actor: 'runner',
      ...
      expect(messageItem.querySelector('[data-message-actor]')?.textContent).toBe('runner');
    });

    it('passes activeRunView only to the active runner message', () => {
      render(
        <MessageList
          messages={mockMessages}
    ...
    it('passes activeRunView to a synthetic streaming runner item when it is the active run message', () => {
      render(
        <MessageList
          messages={[]}
          streamingMessageId="msg-streaming"
    ...
    it('renders a pending active run runner item when the active run message is not in messages yet', () => {
      render(
        <MessageList
          messages={[]}
          streamingMessageId={null}
      ...
      expect(pendingItem.querySelector('[data-message-actor]')).toHaveTextContent(
        'runner',
      );
      expect(
...
Second chunk"
  ...
  describe('Edge Cases', () => {
    it('handles messages with empty content', () => {
      const emptyMessages: TaskActivityItem[] = [
        {
          id: 'msg-1',
          task_id: 'task-1',
          kind: 'user_intent',
    ...
    it('handles large number of messages', () => {
      const manyMessages: TaskActivityItem[] = Array.from({ length: 100 }, (_, i) => ({
        id: `msg-${i}`,
        task_id: 'task-1',
        kind: i % 2 === 0 ? 'user_intent' : 'runner_output',
        actor: i % 2 === 0 ? 'user' : 'runner',
        content: `Message ${i}`,
